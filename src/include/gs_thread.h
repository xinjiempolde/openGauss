...
typedef enum knl_thread_role {
    ...
    // Add by singheart
    NEU_TRANSACTION_SENDER,
    NEU_RESPONSE_RECEIVER,
    NEU_APPLY_LOG_RECEIVER,
...
typedef struct ThreadArg {
    void* (*m_taskRoutine)(
        void*); /* Task function caller passed in by the caller. */
    struct ThreadArg*
        next; /* if next is INVALID_NEXT_ID, the ThreadArg is malloc when create a thread */
...
extern int gs_thread_create(gs_thread_t* thread, void* (*taskRoutine)(void*),
                            int argc, void* argv);

extern int gs_thread_join(gs_thread_t thread, void** value_ptr);

extern void gs_thread_args_pool_init(unsigned long pool_size,
                                     unsigned long backend_para_size);
