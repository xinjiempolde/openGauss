...
// add by singheart
#include "postmaster/neu_apply_log_receiver.h"
#include "postmaster/neu_response_receiver.h"
#include "postmaster/neu_transaction_sender.h"
...
#define IS_FD_TO_RECV_GSSOCK(fd)                       \
    ((fd) == t_thrd.postmaster_cxt.sock_for_libcomm || \
     (fd) == t_thrd.libpq_cxt.listen_fd_for_recv_flow_ctrl)
            ...
            if (t_thrd.storage_cxt.cancel_from_timeout) {                      \
                ereport(                                                       \
                    ...
                    (errcode(ERRCODE_QUERY_CANCELED),                          \
                     errmsg("terminate because pooler connect timeout(%ds) "   \
                            "when process startup packet",                     \
...
#define WalRcvIsOnline()                        \
    ((g_instance.pid_cxt.WalReceiverPID != 0 && \
      t_thrd.walreceiverfuncs_cxt.WalRcv &&     \
      t_thrd.walreceiverfuncs_cxt.WalRcv->isRuning))

#define DataRcvIsOnline()                                                      \
    ((IS_DN_DUMMY_STANDYS_MODE() ? (g_instance.pid_cxt.DataReceiverPID != 0 && \
                                    t_thrd.datareceiver_cxt.DataRcv &&         \
...
static void SetWalsndsNodeState(ClusterNodeState requester,
                                ClusterNodeState others);
...
static void LogChildExit(int lev, const char* procname, ThreadId pid,
                         int exitstatus);
...
static void report_fork_failure_to_client(Port* port, int errnum,
                                          const char* specialErrorInfo = NULL);
...
static bool CreateOptsFile(int argc, const char* argv[],
...
void ReportAlarmAbnormalDataHAInstListeningSocket() {
    ...
    AlarmItemInitialize(alarmItem, ALM_AI_AbnormalDataHAInstListeningSocket,
                        ALM_AS_Reported, NULL);
    // fill the alarm message
    WriteAlarmAdditionalInfo(
        &tempAdditionalParam, g_instance.attr.attr_common.PGXCNodeName, "", "",
        alarmItem, ALM_AT_Fault, g_instance.attr.attr_common.PGXCNodeName);
    // report the alarm
...
char* pg_strdup(const char* string) {
    ...
    if (NULL == string) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_OPERATION),
                        errmsg("pg_strdup: cannot duplicate null pointer "
                               "(internal error)\n")));
    }
    tmp = strdup(string);
    if (NULL == tmp) {
        ereport(ERROR,
                (errcode(ERRCODE_SYSTEM_ERROR), errmsg("out of memory\n")));
    }
    return tmp;
}
...
static void InitDnHashTable(void) {
    ...
    g_instance.comm_cxt.usedDnSpace =
        hash_create("SQL used space on datanode",
                    g_instance.attr.attr_network.MaxConnections, &hctl,
...
bool GetLogicClusterFromConfig(const char* filepath, char* lcname) {
    ...
    switch (status) {
        ...
        case READ_FILE_ERROR:
            ereport(LOG, (errmsg("could not open or read logic cluster static "
                                 "config files: %s",
                                 gs_strerror(err_no))));
            return false;
        case OUT_OF_MEMORY:
    ...
    if (g_logicClusterName[0] == '\0') {
        int ret = snprintf_s(g_logicClusterName, CLUSTER_NAME_LEN,
                             CLUSTER_NAME_LEN - 1, "elastic_group");
...
void GetLogicClusterForAlarm(char* lcname) {
    ...
    ret = snprintf_s(LCListfile, MAXPGPATH, MAXPGPATH - 1, "%s/bin/%s",
                     gausshome, LOGIC_CLUSTER_LIST);
    securec_check_ss_c(ret, "\0", "\0");
    pfree(gausshome);
    gausshome = NULL;
...
void get_coredump_pattern_path(char* path, Size len) {
    ...
    if (0 != stat(path, &stat_buf) || !S_ISDIR(stat_buf.st_mode) ||
        0 != access(path, W_OK)) {
...
bool SetDBStateFileState(DbState state, bool optional) {
    ...
    if (strlen(gaussdb_state_file) > 0) {
        ...
        rc = snprintf_s(temppath, MAXPGPATH, MAXPGPATH - 1, "%s.temp",
                        gaussdb_state_file);
        ...
        if (fd == -1) {
            if (errno == ENOENT && optional) {
                write_stderr(
                    "gaussdb.state does not exist, and skipt setting since it "
                    "is optional.");
                return true;
            } else {
        ...
        if (rename(temppath, gaussdb_state_file) != 0) {
            write_stderr("Failed to rename gaussdb.state.temp: %s",
                         gs_strerror(errno));
            return false;
        }
    }
...
void SetShmemCxt(void) {
    ...
    if (g_threadPoolControler != NULL) {
        ...
        g_instance.shmem_cxt.ThreadPoolGroupNum =
            g_threadPoolControler->GetGroupNum();
    } else {
    ...
    g_instance.shmem_cxt.MaxConnections =
        Max((g_instance.attr.attr_network.MaxConnections +
    ...
    g_instance.shmem_cxt.MaxReserveBackendId =
        g_instance.attr.attr_sql.job_queue_processes +
        g_instance.attr.attr_storage.autovacuum_max_workers +
...
static void print_port_info() {
    ...
    appendStringInfo(&strinfo, "lsof -i:%d",
    ...
    appendStringInfo(&strinfo, "netstat -anp | grep %d",
                     g_instance.attr.attr_network.PostPortNumber);
    fp = popen(strinfo.data, "r");
    if (fp == NULL) {
...
int PostmasterMain(int argc, char* argv[]) {
     ...
     */
    t_thrd.mem_cxt.postmaster_mem_cxt = AllocSetContextCreate(
        t_thrd.top_mem_cxt, "Postmaster", ALLOCSET_DEFAULT_MINSIZE,
    ...
    while (
        (opt = getopt_r(argc, argv,
                        "A:B:bc:C:D:d:EeFf:h:ijk:lM:N:nOo:Pp:Rr:S:sTt:u:W:-:",
                        &optCtxt)) != -1) {
        switch (opt) {
            case 'A':
                SetConfigOption("debug_assertions", optCtxt.optarg,
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;

            case 'B':
                SetConfigOption("shared_buffers", optCtxt.optarg,
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;
            ...
            case 'd':
                set_debug_options(atoi(optCtxt.optarg), PGC_POSTMASTER,
                                  PGC_S_ARGV);
                break;

            case 'E':
                SetConfigOption("log_statement", "all", PGC_POSTMASTER,
                                PGC_S_ARGV);
                break;

            case 'e':
                SetConfigOption("datestyle", "euro", PGC_POSTMASTER,
                                PGC_S_ARGV);
                break;
            ...
            case 'f':
                if (!set_plan_disabling_options(optCtxt.optarg, PGC_POSTMASTER,
                                                PGC_S_ARGV)) {
            ...
            case 'h':
                SetConfigOption("listen_addresses", optCtxt.optarg,
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;

            case 'i':
                SetConfigOption("listen_addresses", "*", PGC_POSTMASTER,
                                PGC_S_ARGV);
                break;
            ...
            case 'k':
                SetConfigOption("unix_socket_directory", optCtxt.optarg,
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;
            ...
            case 'O':
                SetConfigOption("allow_system_table_mods", "true",
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;
            ...
            case 'o':
                ...
                rc = snprintf_s(
                    g_instance.ExtraOptions + strlen(g_instance.ExtraOptions),
                    sizeof(g_instance.ExtraOptions) -
                        strlen(g_instance.ExtraOptions),
                    sizeof(g_instance.ExtraOptions) -
                        strlen(g_instance.ExtraOptions) - 1,
                    " %s", optCtxt.optarg);
                securec_check_ss(rc, "", "");
                break;
            ...
            case 'p':
                SetConfigOption("port", optCtxt.optarg, PGC_POSTMASTER,
                                PGC_S_ARGV);
                break;
#ifdef ENABLE_MULTIPLE_NODES
            case 'R':
            ...
            case 'S':
                SetConfigOption("work_mem", optCtxt.optarg, PGC_POSTMASTER,
                                PGC_S_ARGV);
                break;
            ...
            case 'u':
                ...
                pg_atomic_write_u32(&WorkingGrandVersionNum,
                                    (uint32)strtoul(optCtxt.optarg, NULL, 10));
                if (errno != 0 || pg_atomic_read_u32(&WorkingGrandVersionNum) >
                                      GRAND_VERSION_NUM) {
                    write_stderr(
                        "%s: invalid argument for option -u: \"%s\", "
                        "GRAND_VERSION_NUM is %u\n",
                        progname, optCtxt.optarg, (uint32)GRAND_VERSION_NUM);
                    ExitPostmaster(1);
                } else if (pg_atomic_read_u32(&WorkingGrandVersionNum) ==
            ...
            case 'W':
                SetConfigOption("post_auth_delay", optCtxt.optarg,
                                PGC_POSTMASTER, PGC_S_ARGV);
                break;
            ...
            case '-': {
                ...
                if (opt == '-' && (strcmp(name, "coordinator") == 0 ||
                                   strcmp(name, "datanode") == 0)) {
                    ereport(FATAL, (errcode(ERRCODE_SYNTAX_ERROR),
                                    errmsg("Single node mode: must start as "
                                           "single node (--single_node)\n")));
                }
#endif
                /* A Coordinator is being activated */
                if (name != NULL && strcmp(name, "coordinator") == 0 &&
                    value == NULL)
                    g_instance.role = VCOORDINATOR;
                else if (name != NULL && strcmp(name, "datanode") == 0 &&
                         value == NULL)
                    g_instance.role = VDATANODE;
                /* A SingleDN mode is being activated */
                else if (name != NULL && strcmp(name, "single_node") == 0 &&
                         value == NULL) {
                    g_instance.role = VSINGLENODE;
                    useLocalXid = true;
                } else if (name != NULL && strcmp(name, "restoremode") == 0 &&
                           value == NULL) {
                    /*
                    ...
                    g_instance.role = VDATANODE;
                           ...
                           0 == strncasecmp(name, "fenced", strlen(name)) &&
                           value == NULL)
                    FencedUDFMasterMode = true;
                else if (name != NULL &&
                         strlen(name) == SECURITY_MODE_NAME_LEN &&
                         strncmp(name, SECURITY_MODE_NAME,
                                 SECURITY_MODE_NAME_LEN) == 0 &&
                         value == NULL) {
    ...
    if (!IS_PGXC_COORDINATOR && !IS_PGXC_DATANODE && !FencedUDFMasterMode) {
        write_stderr(
            "%s: Postgres-XC: must start as either a Coordinator "
            "(--coordinator) or Datanode (--datanode)\n",
            progname);
        ExitPostmaster(1);
    }
    ...
    if ((g_instance.attr.attr_security.transparent_encrypted_string != NULL &&
        ...
        (g_instance.attr.attr_security.transparent_encrypt_kms_region != NULL &&
         g_instance.attr.attr_security.transparent_encrypt_kms_region[0] !=
             '\0')) {
        isSecurityMode = true;
    }
    ...
    if (FencedUDFMasterMode) {
        ...
        gs_thread_args_pool_init(GLOBAL_ALL_PROCS + EXTERN_SLOTS_NUM,
        ...
        int rc1 = snprintf_s(gaussdb_state_file, sizeof(gaussdb_state_file),
                             MAXPGPATH - 1, "%s/gaussdb.state",
        ...
        /* For debugging: display postmaster environment */
        {
            ...
            ereport(DEBUG3, (errmsg_internal(
                                "%s: PostmasterMain: initial environment dump:",
                                progname)));
            ereport(
                DEBUG3,
                (errmsg_internal("-----------------------------------------")));
        ...
        rc = memcpy_s(g_alarmComponentPath, MAXPGPATH - 1, Alarm_component,
            ...
            foreach (l, elemlist) {
                ...
                if (strcmp(curhost, "*") == 0)
                    status = StreamServerPort(
                        ...
                        t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN, true,
                        true, false);
                else
                    status = StreamServerPort(
                        AF_UNSPEC, curhost,
                ...
                if (t_thrd.xlog_cxt.server_mode == NORMAL_MODE ||
                    ...
                    if (g_instance.attr.attr_network.PoolerPort !=
                        (g_instance.attr.attr_network.PostPortNumber + 1)) {
                        ereport(FATAL, (errmsg("pooler_port must equal to gsql "
                                               "listen port plus one!")));
                    }
                    ...
                    if (strcmp(curhost, "*") == 0) {
                        status = StreamServerPort(
                            ...
                            g_instance.attr.attr_network.UnixSocketDir,
                            t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN,
                            false, false, false);
                    } else {
                        status = StreamServerPort(
                            AF_UNSPEC, curhost,
                    ...
                    if (status != STATUS_OK)
                        ereport(
                            FATAL,
                            (errmsg("could not create ha listen socket for "
                                    "\"%s:%d\"",
                                    curhost,
                                    g_instance.attr.attr_network.PoolerPort)));
        ...
        if (t_thrd.xlog_cxt.server_mode != NORMAL_MODE) {
            ...
            for (i = 1; i < MAX_REPLNODE_NUM; i++) {
                if (t_thrd.postmaster_cxt.ReplConnArray[i] != NULL) {
                    if (!listen_addr_saved &&
                        !IsInplicitIp(t_thrd.postmaster_cxt.ReplConnArray[i]
                                          ->localhost)) {
                    ...
                    if (IsAlreadyListen(
                            t_thrd.postmaster_cxt.ReplConnArray[i]->localhost,
                            t_thrd.postmaster_cxt.ReplConnArray[i]
                                ->localport)) {
                        success++;
                        continue;
                    }
                    ...
                    status = StreamServerPort(
                        AF_UNSPEC,
                        t_thrd.postmaster_cxt.ReplConnArray[i]->localhost,
                        (unsigned short)t_thrd.postmaster_cxt.ReplConnArray[i]
                            ->localport,
                        g_instance.attr.attr_network.UnixSocketDir,
                        t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN, false,
                        false, false);
                    if (status == STATUS_OK) {
                        success++;
                    } else {
            ...
            if (success == 0) {
                ...
                ereport(WARNING,
                        (errmsg("could not create any HA TCP/IP sockets")));
            }
        ...
        if (g_instance.attr.attr_common.enable_bonjour &&
            t_thrd.postmaster_cxt.ListenSocket[0] != PGINVALID_SOCKET) {
            ...
            err = DNSServiceRegister(
                &bonjour_sdref, 0, 0, g_instance.attr.attr_common.bonjour_name,
                "_postgresql._tcp.", NULL, NULL,
                htons(g_instance.attr.attr_network.PostPortNumber), 0, NULL,
                NULL, NULL);
        ...
        if (!dummyStandbyMode) {
            ...
            status = StreamServerPort(
                ...
                g_instance.attr.attr_network.UnixSocketDir,
                t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN, false, true,
                false);
            ...
            status = StreamServerPort(
                ...
                g_instance.attr.attr_network.UnixSocketDir,
                t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN, false, false,
                false);
            ...
            if (g_instance.attr.attr_storage.comm_cn_dn_logic_conn &&
                ...
                if (status != STATUS_OK)
                    ereport(
                        WARNING,
                        (errmsg(
                            "could not create Unix-domain for comm  socket")));
            }
        }
#endif
        ...
        if (!listen_addr_saved) {
            ...
            ereport(
                WARNING,
                (errmsg(
                    "No explicit IP is configured for listen_addresses GUC.")));
        }
        ...
        if (g_instance.attr.attr_common.enable_thread_pool) {
            ...
            if (!g_instance.attr.attr_storage.comm_cn_dn_logic_conn &&
                !dummyStandbyMode) {
                g_threadPoolControler = (ThreadPoolControler*)New(
                    INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_EXECUTOR))
                    ThreadPoolControler();
        ...
        ereport(LOG,
                (errmsg("create gaussdb state file success: db "
                        "state(STARTING_STATE), server mode(%s)",
                        wal_get_role_string(
         ...
         */
        if (!CreateOptsFile(argc, (const char**)argv,
    ...
    if (hotpatch_remove_signal_file(t_thrd.proc_cxt.DataDir) == HP_OK) {
        int ret;
        ret = hotpatch_init(t_thrd.proc_cxt.DataDir,
                            (HOTPATCH_LOG_FUNC)gs_hotpatch_log_callback);
        if (ret != HP_OK) {
    ...
    if (g_instance.attr.attr_common.external_pid_file) {
        FILE* fpidfile =
            fopen(g_instance.attr.attr_common.external_pid_file, "w");
        ...
        if (fpidfile != NULL) {
            ...
            if (chmod(g_instance.attr.attr_common.external_pid_file,
                      S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) != 0)
                write_stderr(
                    "%s: could not change permissions of external PID file "
                    "\"%s\": %s\n",
                    progname, g_instance.attr.attr_common.external_pid_file,
                    gs_strerror(errno));
        } else
    ...
    ProcSubXidCacheContext = AllocSetContextCreate(
        g_instance.instance_context, "ProcSubXidCacheContext",
     ...
     */
    StreamInfoContext = AllocSetContextCreate(
        g_instance.instance_context, "StreamInfoContext",
        ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT);
    ...
    if ((!IS_SINGLE_NODE) &&
        ...
        if (status != STATUS_OK)
            ereport(FATAL, (errmsg("Init libcomm for stream failed, maybe "
    ...
    gs_lock_test_and_set_64(&g_instance.stat_cxt.NodeStatResetTime,
                            GetCurrentTimestamp());
...
static void getInstallationPaths(const char* argv0) {
    ...
    if (find_other_exec(argv0, "gaussdb", PG_BACKEND_VERSIONSTR,
                        t_thrd.proc_cxt.postgres_exec_path) < 0) {
        ereport(FATAL,
    ...
    if (pdir == NULL) {
        ereport(ERROR,
                (errcode_for_file_access(),
                 ...
                 errhint("This may indicate an incomplete PostgreSQL "
                         "installation, or that the file \"%s\" has been "
...
#define BUILD_TAG_START "build_completed.start"
    ...
    int rc = snprintf_s(identFile, sizeof(identFile), MAXPGPATH - 1, "%s/%s",
                        t_thrd.proc_cxt.DataDir, BUILD_TAG_START);
    securec_check_intval(rc, , );
    if (0 == stat(identFile, &stat_buf)) {
        write_stderr(
            "%s: Uncompleted build is detected, please build again and then "
            "start database after its success.\n",
            progname);
        ExitPostmaster(2);
    }
    ...
    if (stat_buf.st_uid != geteuid())
        ereport(FATAL, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg("data directory \"%s\" has wrong ownership",
                               t_thrd.proc_cxt.DataDir),
                        errhint("The server must be started by the user that "
    ...
    int ret = snprintf_s(path, sizeof(path), MAXPGPATH - 1,
                         "%s/global/pg_control", t_thrd.proc_cxt.DataDir);
    ...
    if (fp == NULL) {
        write_stderr(
            ...
            progname, t_thrd.proc_cxt.DataDir, path, gs_strerror(errno));
        ExitPostmaster(2);
    }
...
static void CheckExtremeRtoGUCConflicts(void) {
    ...
    if ((g_instance.attr.attr_storage.recovery_parse_workers > 1) &&
        ...
        ereport(WARNING, (errcode(ERRCODE_SYSTEM_ERROR),
                          errmsg("when starting as dummy_standby mode, we "
                                 "couldn't support extreme rto."),
    ...
    if ((g_instance.attr.attr_storage.recovery_parse_workers > 1) &&
        ...
        ereport(
            ...
            (errcode(ERRCODE_SYSTEM_ERROR),
             errmsg("when starting extreme rto, wal receiver buf should not "
                    "smaller than %dMB",
...
static void CheckRecoveryParaConflict() {
    if (g_instance.attr.attr_storage.max_recovery_parallelism >
            RECOVERY_PARALLELISM_DEFAULT &&
        IS_DN_DUMMY_STANDYS_MODE()) {
        ereport(WARNING, (errmsg("when starting as dummy_standby mode, we "
                                 "couldn't support parallel redo, down it")));
...
static void CheckGUCConflictsMaxConnections() {
    if (g_instance.attr.attr_network.ReservedBackends >=
        g_instance.attr.attr_network.MaxConnections) {
        write_stderr(
            "%s: sysadmin_reserved_connections must be less than "
            "max_connections\n",
            progname);
        ExitPostmaster(1);
    }

    if (g_instance.attr.attr_storage.max_wal_senders >=
        g_instance.attr.attr_network.MaxConnections) {
...
static void CheckGUCConflicts(void) {
    ...
    if (u_sess->attr.attr_common.XLogArchiveMode &&
        g_instance.attr.attr_storage.wal_level == WAL_LEVEL_MINIMAL)
        ereport(ERROR,
                (errcode(ERRCODE_SYSTEM_ERROR),
                 errmsg("WAL archival (archive_mode=on) requires wal_level "
    ...
    if (g_instance.attr.attr_storage.max_wal_senders > 0 &&
        ...
        ereport(ERROR, (errcode(ERRCODE_SYSTEM_ERROR),
                        errmsg("WAL streaming (max_wal_senders > 0) requires "
                               "wal_level \"archive\", \"hot_standby\" or "
                               "\"logical\"")));
    ...
    if (g_instance.attr.attr_storage.EnableHotStandby == true &&
        ...
        ereport(ERROR, (errcode(ERRCODE_SYSTEM_ERROR),
                        errmsg("hot standby is not possible because wal_level "
                               "was not set to \"hot_standby\""),
                        errhint("Either set wal_level to \"hot_standby\", or "
    ...
    if ((g_instance.attr.attr_storage.wal_level == WAL_LEVEL_MINIMAL ||
        ...
        ereport(ERROR,
                (errcode(ERRCODE_SYSTEM_ERROR),
                 errmsg("when starting as dual mode, we must ensure wal_level "
                        "was not \"minimal\" and max_wal_senders "
                        "was set at least 1")));

    if (u_sess->attr.attr_storage.enable_data_replicate &&
        IS_DN_MULTI_STANDYS_MODE()) {
        /* when init, we should force change the option */
        ereport(LOG, (errmsg("when starting as multi_standby mode, we couldn't "
                             "support data replicaton.")));
    ...
    if (g_instance.attr.attr_storage.enable_mix_replication &&
        ...
        write_stderr(
            "%s: the data queue buffer size must be larger than the wal sender "
            "max send size for the "
...
static int ServerLoop(void) {
    ...
    int nSockets;
    uint64 this_start_poll_time, last_touch_time, last_start_loop_time,
        last_start_poll_time;
    ...
    FD_ZERO(&readmask);
    last_start_loop_time = last_touch_time = last_start_poll_time =
        mc_timers_us();
    ...
    if (!save_backend_variables_for_callback_thread()) {
        ereport(LOG,
                (errmsg("save_backend_variables_for_callback_thread error")));
        return STATUS_ERROR;
    }
    ...
    for (;;) {
         ...
         */
        int ss_rc = memcpy_s((char*)&rmask, sizeof(fd_set), (char*)&readmask,
                             sizeof(fd_set));
        securec_check(ss_rc, "\0", "\0");
        ...
        if ((this_start_poll_time - last_start_loop_time) != 0) {
            gs_set_libcomm_used_rate(
                (this_start_poll_time - last_start_poll_time) * 100 /
                (this_start_poll_time - last_start_loop_time));
        }
         ...
         */
        if (this_start_poll_time - last_start_poll_time >
            PM_BUSY_ALARM_USED_US) {
            /* Prevent interrupts while cleaning up */
            HOLD_INTERRUPTS();
            ereport(WARNING,
                    (errmsg("postmaster is busy, this cycle used %lu seconds.",
                            (this_start_poll_time - last_start_loop_time) /
                                PM_BUSY_ALARM_US)));
...
#else
            int ss_rc = memcpy_s((char*)&rmask, sizeof(fd_set),
                                 (char*)&readmask, sizeof(fd_set));
            securec_check(ss_rc, "\0", "\0");
            ...
            if (errno != EINTR && errno != EWOULDBLOCK) {
                ereport(LOG,
                        (errcode_for_socket_access(),
                         errmsg("select()/poll() failed in postmaster: %m")));
                return STATUS_ERROR;
            }
        }
                ...
                if (FD_ISSET(t_thrd.postmaster_cxt.ListenSocket[i], &rmask)) {
                    ...
                    if (port != NULL) {
                        ...
                        bool isConnectHaPort =
                            (i < MAXLISTEN) &&
                            (t_thrd.postmaster_cxt.listen_sock_type[i] ==
                             HA_LISTEN_SOCKET);
                        /*
                        ...
                        if (threadPoolActivated && !isConnectHaPort) {
                            result =
                                g_threadPoolControler->DispatchSession(port);
                        } else {
        ...
        ADIO_RUN() {
            if (!g_instance.pid_cxt.AioCompleterStarted && !dummyStandbyMode) {
                ...
                if ((aioStartErr = AioCompltrStart()) == 0) {
                    ...
                    ereport(LOG,
                            (errmsg_internal(
                                "Cannot start AIO completer threads error=%d",
                                aioStartErr)));
                    /*
                    ...
                    HandleChildCrash(g_instance.pid_cxt.AioCompleterStarted, 1,
                                     "AIO process");
                }
            }
        }
        ...
        if (g_instance.pid_cxt.PgAuditPID == 0 &&
            ...
            ereport(LOG, (errmsg("auditor process started, pid=%lu",
                                 g_instance.pid_cxt.PgAuditPID)));
        }
#else
        if (g_instance.pid_cxt.PgAuditPID == 0 &&
            u_sess->attr.attr_security.Audit_enabled && pmState == PM_RUN &&
            !dummyStandbyMode) {
            g_instance.pid_cxt.PgAuditPID = pgaudit_start();
            ereport(LOG, (errmsg("auditor process started, pid=%lu",
                                 g_instance.pid_cxt.PgAuditPID)));
        ...
        if (g_instance.pid_cxt.PgAuditPID != 0 &&
            ...
            ereport(LOG, (errmsg("parameter audit_enabled is set to false, "
                                 "terminate auditor process.")));
        }
        ...
        if (g_instance.pid_cxt.ReaperBackendPID == 0)
            g_instance.pid_cxt.ReaperBackendPID =
        ...
        if (pmState == PM_RUN || pmState == PM_RECOVERY ||
            ...
            if (g_instance.pid_cxt.CheckpointerPID == 0 && !dummyStandbyMode)
                g_instance.pid_cxt.CheckpointerPID =
                    initialize_util_thread(CHECKPOINT_THREAD);

            if (g_instance.pid_cxt.BgWriterPID == 0 && !dummyStandbyMode &&
                !g_instance.attr.attr_storage.enableIncrementalCheckpoint) {
                g_instance.pid_cxt.BgWriterPID =
                    initialize_util_thread(BGWRITER);
            }
            ...
            if (g_instance.pid_cxt.CBMWriterPID == 0 && !dummyStandbyMode &&
                ...
                g_instance.pid_cxt.CBMWriterPID =
                    initialize_util_thread(CBMWRITER);

            if (!dummyStandbyMode &&
                g_instance.attr.attr_storage.enableIncrementalCheckpoint) {
                for (int i = 0;
                     i < g_instance.attr.attr_storage.pagewriter_thread_num;
                     i++) {
                    if (g_instance.pid_cxt.PageWriterPID[i] == 0) {
                        g_instance.pid_cxt.PageWriterPID[i] =
                            initialize_util_thread(PAGEWRITER_THREAD);
                    }
                }
                int thread_num =
                ...
                for (int i = 0; i < thread_num; i++) {
                    if (g_instance.pid_cxt.CkptBgWriterPID[i] == 0) {
                        g_instance.pid_cxt.CkptBgWriterPID[i] =
                            initialize_util_thread(BGWRITER);
                    }
                }
            }

            if (g_instance.pid_cxt.RemoteServicePID == 0 && !dummyStandbyMode &&
                IS_PGXC_DATANODE &&
                t_thrd.postmaster_cxt.HaShmData->current_mode != NORMAL_MODE &&
                !IS_DN_WITHOUT_STANDBYS_MODE() && IsRemoteReadModeOn() &&
                get_cur_repl_num())
                g_instance.pid_cxt.RemoteServicePID =
                    initialize_util_thread(RPC_SERVICE);
        }
        /*
        ...
        if (g_instance.pid_cxt.WalWriterAuxiliaryPID == 0 &&
            (pmState == PM_RUN ||
             ((pmState == PM_HOT_STANDBY || pmState == PM_RECOVERY) &&
              g_instance.pid_cxt.WalRcvWriterPID != 0 &&
              t_thrd.postmaster_cxt.HaShmData->current_mode == STANDBY_MODE))) {
            g_instance.pid_cxt.WalWriterAuxiliaryPID =
                initialize_util_thread(WALWRITERAUXILIARY);
            ereport(LOG,
                    (errmsg("ServerLoop create WalWriterAuxiliary(%lu) for "
                            "pmState:%u, ServerMode:%u.",
                            g_instance.pid_cxt.WalWriterAuxiliaryPID, pmState,
                            t_thrd.postmaster_cxt.HaShmData->current_mode)));
        }
        ...
        if (!u_sess->attr.attr_storage.enable_cbm_tracking &&
            g_instance.pid_cxt.CBMWriterPID != 0 && pmState == PM_RUN) {
            ereport(LOG, (errmsg("stop cbm writer thread because "
                                 "enable_cbm_tracking is switched off, "
        ...
        if (!u_sess->proc_cxt.IsBinaryUpgrade &&
            ...
            (AutoVacuumingActive() ||
             t_thrd.postmaster_cxt.start_autovac_launcher) &&
            pmState == PM_RUN && !dummyStandbyMode &&
            u_sess->attr.attr_common.upgrade_mode != 1 &&
            !InplaceUpgradePrecommit) {
            g_instance.pid_cxt.AutoVacPID =
                initialize_util_thread(AUTOVACUUM_LAUNCHER);
        ...
        if (g_instance.pid_cxt.PgJobSchdPID == 0 && pmState == PM_RUN &&
            (g_instance.attr.attr_sql.job_queue_processes ||
             t_thrd.postmaster_cxt.start_job_scheduler) &&
            u_sess->attr.attr_common.upgrade_mode != 1) {
            g_instance.pid_cxt.PgJobSchdPID =
                initialize_util_thread(JOB_SCHEDULER);

            if (g_instance.pid_cxt.PgJobSchdPID != 0) {
                t_thrd.postmaster_cxt.start_job_scheduler =
                    false; /* signal processed */
                ereport(LOG, (errmsg("job scheduler started, pid=%lu",
                                     g_instance.pid_cxt.PgJobSchdPID)));
            }
        }
#ifdef ENABLE_MULTIPLE_NODES
        if ((IS_PGXC_COORDINATOR) &&
            g_instance.pid_cxt.CommPoolerCleanPID == 0 && pmState == PM_RUN &&
            u_sess->attr.attr_common.upgrade_mode != 1) {
            StartPoolCleaner();
        }
#endif
        ...
        if (XLogArchivingActive() && g_instance.pid_cxt.PgArchPID == 0 &&
            !dummyStandbyMode) {
            if (pmState == PM_RUN) {
        ...
        if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) &&
            g_instance.pid_cxt.SnapshotPID == 0 && pmState == PM_RUN)
            g_instance.pid_cxt.SnapshotPID = snapshot_start();

        if (ENABLE_ASP && g_instance.pid_cxt.AshPID == 0 && pmState == PM_RUN &&
            !dummyStandbyMode)
        ...
        if (ENABLE_STATEMENT_TRACK && g_instance.pid_cxt.StatementPID == 0 &&
            ...
            g_instance.pid_cxt.StatementPID =
                initialize_util_thread(TRACK_STMT_WORKER);

        if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) &&
            g_instance.pid_cxt.PercentilePID == 0 && pmState == PM_RUN)
            g_instance.pid_cxt.PercentilePID =
                initialize_util_thread(PERCENTILE_WORKER);

        /* if workload manager is off, we still use this thread to build user hash table */
        if ((ENABLE_WORKLOAD_CONTROL || !WLMIsInfoInit()) &&
            g_instance.pid_cxt.WLMCollectPID == 0 && pmState == PM_RUN &&
            !dummyStandbyMode)
            g_instance.pid_cxt.WLMCollectPID =
                initialize_util_thread(WLM_WORKER);

        if (ENABLE_WORKLOAD_CONTROL &&
            (g_instance.pid_cxt.WLMMonitorPID == 0) && (pmState == PM_RUN) &&
            !dummyStandbyMode)
            g_instance.pid_cxt.WLMMonitorPID =
                initialize_util_thread(WLM_MONITOR);

        if (ENABLE_WORKLOAD_CONTROL &&
            (g_instance.pid_cxt.WLMArbiterPID == 0) && (pmState == PM_RUN) &&
            !dummyStandbyMode)
            g_instance.pid_cxt.WLMArbiterPID =
                initialize_util_thread(WLM_ARBITER);

        if (IS_PGXC_COORDINATOR &&
            g_instance.attr.attr_sql.max_resource_package &&
            (g_instance.pid_cxt.CPMonitorPID == 0) && (pmState == PM_RUN) &&
            !dummyStandbyMode)
            g_instance.pid_cxt.CPMonitorPID =
                initialize_util_thread(WLM_CPMONITOR);
        ...
        if (GTM_LITE_CN && g_instance.csnminsync_cxt.is_fcn_ccn &&
            ...
            g_instance.pid_cxt.CsnminSyncPID =
                initialize_util_thread(CSNMIN_SYNC);
        }

        /* If we have lost the barrier creator thread, try to start a new one */
        if (START_BARRIER_CREATOR &&
            g_instance.pid_cxt.BarrierCreatorPID == 0 && pmState == PM_RUN &&
            XLogArchivingActive()) {
            obs_slot = getObsReplicationSlot();
            if (obs_slot != NULL) {
                g_instance.pid_cxt.BarrierCreatorPID =
                    initialize_util_thread(BARRIER_CREATOR);
            }
        }
        ...
        if (IS_PGXC_DATANODE && g_instance.attr.attr_common.enable_tsdb &&
            ...
            g_instance.pid_cxt.TsCompactionPID =
                initialize_util_thread(TS_COMPACTION);
        }
        ...
        if (IS_PGXC_DATANODE && g_instance.attr.attr_common.enable_tsdb &&
            ...
            g_instance.pid_cxt.TsCompactionAuxiliaryPID =
                initialize_util_thread(TS_COMPACTION_AUXILIAY);
        }
#endif /* ENABLE_MULTIPLE_NODES */
        ...
        if (pmState == PM_RUN) {
            if (t_thrd.postmaster_cxt.audit_primary_failover) {
                int rcs = snprintf_s(
                    details, sizeof(details), sizeof(details) - 1,
...
int ProcessStartupPacket(Port* port, bool SSLdone) {
    ...
    struct timeval tv = {
        tvFactor * u_sess->attr.attr_network.PoolerConnectTimeout, 0};
    ...
    if (IS_PGXC_COORDINATOR &&
        ...
        if (getsockopt(port->sock, SOL_SOCKET, SO_RCVTIMEO, &oldTv,
                       &oldTvLen)) {
            ereport(LOG, (errmsg("getsockopt(SO_RCVTIMEO) failed: %m")));
            return STATUS_ERROR;
        }

        if (setsockopt(port->sock, SOL_SOCKET, SO_RCVTIMEO, &tv,
    ...
    if (pq_getbytes((char*)&len, 4) == EOF) {
         ...
         */
        if (!SSLdone)
            ereport(DEBUG1, (errcode(ERRCODE_PROTOCOL_VIOLATION),
    ...
    if (pq_getbytes((char*)buf, len) == EOF) {
        ereport(
            ...
            (errcode(ERRCODE_PROTOCOL_VIOLATION),
             errmsg(
                 "incomplete startup packet, remote_host[%s], remote_port[%s].",
    ...
    switch (port->canAcceptConnections) {
        ...
        case CAC_TOOMANY:
            ereport(elevel, (errcode(ERRCODE_TOO_MANY_CONNECTIONS),
    ...
    if (proto == NEGOTIATE_SSL_CODE && !SSLdone) {
        ...
        if (!g_instance.attr.attr_security.EnableSSL ||
            IS_AF_UNIX(port->laddr.addr.ss_family))
            SSLok = 'N';
        else
    ...
    for (size_t i = 0;
         i < sizeof(protoVersionList) / sizeof(protoVersionList[0]); i++) {
        if (PG_PROTOCOL_MAJOR(proto) == protoVersionList[i][0] &&
            PG_PROTOCOL_MINOR(proto) == protoVersionList[i][1]) {
            findProtoVer = true;
            break;
        }
    }
    ...
    if (PG_PROTOCOL_MAJOR(proto) >= 3) {
        ...
        while (offset < len) {
            ...
            if (strcmp(nameptr, "database") == 0)
                ...
                if (strstr(port->cmdline_options, "xc_maintenance_mode=on") !=
                    NULL) {
                    isMaintenanceConnection = true;
#ifndef ENABLE_MULTIPLE_NODES
                } else if (strstr(port->cmdline_options,
                                  "remotetype=internaltool") != NULL) {
                ...
                if (strcmp(valptr, "data") == 0) {
                    ...
                    if (!IsHAPort(u_sess->proc_cxt.MyProcPort) &&
                        ...
                        ereport(elevel,
                                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                                 errmsg("logical replication should connect HA "
                                        "port in thread_pool")));
                    }
                ...
                if (errno != 0)
                    ereport(
                        ...
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                         errmsg(
                             "invalid value for parameter \"backend_version\""),
                ...
                if (port->SessionVersionNum > GRAND_VERSION_NUM)
                    ereport(elevel, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                                     errmsg("requested backend version is "
                                            "larger than grand version.")));
                ...
                errno = 0;
                const uint32 poolerConnectTimeoutMaxValue =
                    7200; /* Max value of pooler_connect_timeout */
                uint32 poolerConnectTimeout = (uint32)strtoul(valptr, NULL, 10);
                if (errno != 0) {
                    ereport(
                        FATAL,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                         errmsg("invalid value[%d] for parameter "
                                "\"connect_timeout\"",
                                u_sess->attr.attr_network.PoolerConnectTimeout),
                         errhint("Valid values are of uint32 type.")));
                }
                u_sess->attr.attr_network.PoolerConnectTimeout =
                    (poolerConnectTimeout < poolerConnectTimeoutMaxValue)
                        ? poolerConnectTimeout
                        : poolerConnectTimeoutMaxValue;
            } else {
                if (strcmp(nameptr, "application_name") == 0) {
                    ...
                    if (strcmp(valptr, "gs_ctl") == 0) {
                        ...
                        ereport(DEBUG5,
                                (errmsg("secondary standby connected")));
                        ...
                        ereport(DEBUG5,
                                (errmsg("application %s connected", valptr)));
                    }

                    errno_t ssrc =
                        strncpy_s(u_sess->proc_cxt.applicationName, NAMEDATALEN,
                                  valptr, NAMEDATALEN - 1);
                    if (ssrc != EOK) {
                        ereport(WARNING, (errmsg("Save app name %s failed in "
                ...
                port->guc_options =
                    lappend(port->guc_options, pstrdup(nameptr));
         ...
         */
        if (offset != len - 1)
            ereport(elevel, (errcode(ERRCODE_PROTOCOL_VIOLATION),
    ...
    if (clientIsCmAgent || clientIsGsClean || clientIsOM ||
        u_sess->proc_cxt.clientIsGsroach || clientIsWDRXdb ||
        u_sess->proc_cxt.clientIsGsCtl || u_sess->proc_cxt.clientIsGsrewind ||
    ...
    if (clientIsCmAgent) {
        ...
        if (TEST_STUB(DN_CM_NEW_CONN, stub_sleep_emit)) {
            ereport(get_distribute_test_param()->elevel,
                    (errmsg("sleep_emit happen during ProcessStartupPacket  "
                            "time:%ds, stub_name:%s",
    ...
    if (AM_WAL_SENDER) {
        ...
        if (!IS_PGXC_COORDINATOR) {
            ...
            if (NORMAL_MODE == hashmdata->current_mode) {
                if (!u_sess->proc_cxt.clientIsGsBasebackup &&
                    ...
                    ereport(
                        elevel,
                        (errmsg("the current t_thrd.postmaster_cxt.server_mode "
                                "is NORMAL, "
            ...
            if (0 == channel_adapt) {
                ...
                ereport(
                    elevel,
                    (errcode(ERRCODE_CANNOT_CONNECT_NOW),
                     errmsg("the ha connection is not in the channel list")));
            }
        }
    } else {
        if (dummyStandbyMode)
            ereport(elevel,
                    (errcode(ERRCODE_CANNOT_CONNECT_NOW),
                     errmsg("Secondary Standby does not accept connection")));
        ...
        if (!u_sess->proc_cxt.IsInnerMaintenanceTools &&
            !IsConnPortFromCoord(port) &&
            (!IsLocalAddr(port) || !IsLocalPort(port))) {
            ereport(elevel, (errcode(ERRCODE_CANNOT_CONNECT_NOW),
                             errmsg("the local listen ip and port is not for "
                                    "the gsql client")));
        }
    }
    ...
    if ((!AM_WAL_SENDER && !(isMaintenanceConnection &&
                             (clientIsGsql || clientIsCmAgent ||
                              t_thrd.postmaster_cxt.senderToBuildStandby))) ||
...
#else
            if (hashmdata->current_mode == STANDBY_MODE &&
                !g_instance.attr.attr_storage.EnableHotStandby) {
                ereport(
                    elevel,
                    (errcode(ERRCODE_CANNOT_CONNECT_NOW),
    ...
    if ((PM_RUN != pmState) && t_thrd.postmaster_cxt.senderToDummyStandby) {
        ereport(
            ...
            (errcode(ERRCODE_CANNOT_CONNECT_NOW),
             errmsg(
                 "can not accept dummy standby connection in standby mode.")));
        return STATUS_ERROR;
    }
    ...
    if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) &&
        ...
        !IS_AF_UNIX(port->laddr.addr.ss_family) && !SSLdone) {
        /*
        ...
        if (!u_sess->proc_cxt.IsInnerMaintenanceTools &&
            ...
            ereport(
                elevel,
                (errcode(ERRCODE_CANNOT_CONNECT_NOW),
                 errmsg("SSL connection is required by the database system")));
        }
    }
#endif
    ...
    if (port->cmdline_options != NULL &&
        strstr(port->cmdline_options, "remotetype=coordinator") != NULL) {
    ...
    if (isTvSeted && (setsockopt(port->sock, SOL_SOCKET, SO_RCVTIMEO, &oldTv,
                                 oldTvLen) < 0)) {
        ereport(LOG, (errmsg("setsockopt(SO_RCVTIMEO) failed: %m")));
        return STATUS_ERROR;
    }
...
static void processStopRequest(Port* port, void* pkt) {
    ...
    query_id = (((uint64)ntohl(csp->query_id_first)) << 32) +
               (uint32)ntohl(csp->query_id_end);

    if (ENABLE_THREAD_POOL) {
        g_threadPoolControler->GetSessionCtrl()->SendProcSignal(
            logictid, PROCSIG_EXECUTOR_FLAG, query_id);
        return;
    }
...
static void processCancelRequest(Port* port, void* pkt) {
    ...
    if (((unsigned long)cancelAuthCode & 0x1) == 0) {
        if (ENABLE_THREAD_POOL) {
            ...
            ThreadPoolSessControl* sess_ctrl =
                g_threadPoolControler->GetSessionCtrl();
            knl_session_context* sess =
                sess_ctrl->GetSessionByIdx(sess_ctrl_id);

            if (sess == NULL || sess->cancel_key != cancelAuthCode) {
                ereport(
                    LOG,
                    (errmsg("Don't found the match sess, the session slot(%d)",
                            sess_ctrl_id)));
                return;
            }
            ...
            if (err != 0) {
                ereport(WARNING,
                        (errmsg("kill(session %ld, signal %d) failed: \"%s\", "
                                "pmState %d, Demotion %d, Shutdown %d",
                                (long)sess_ctrl_id, SIGINT, gs_strerror(err),
                                pmState, g_instance.demotion, Shutdown)));
            }
        } else {
            ereport(WARNING, (errmsg("Receive invalid cancel key, which "
                                     "suppose to be thread pool mode.")));
        }
    } else {
        int backendSlot = 0;
        // get thread id from logic thread id
        ...
        if (bn == NULL || bn->pid == 0 || bn->pid == InvalidTid) {
            ereport(LOG, (errmsg("Don't found the match process, the backend "
                                 "slot(%d), pid(%lu)",
                                 backendSlot, bn ? bn->pid : 0)));
            return;
        }
        ...
        if (bn->cancel_key == cancelAuthCode) {
            ...
            ereport(
                DEBUG2,
                (errmsg_internal(
                    "processing cancel request: sending SIGINT to process %lu",
                    bn->pid)));
            signal_child(bn->pid, SIGINT);
        } else
            /* Right PID, wrong key: no way, Jose */
            ereport(LOG, (errmsg("wrong key in cancel request for process %lu",
                                 bn->pid)));
    }
}
...
CAC_state canAcceptConnections(bool isSession) {
    ...
    if (pmState != PM_RUN) {
        ...
        else if (g_instance.status > NoShutdown ||
                 g_instance.demotion > NoDemote)
            return CAC_SHUTDOWN; /* shutdown is pending */
        else if (!g_instance.fatal_error &&
...
static Port* ConnCreate(int serverFd) {
    ...
    port = (Port*)MemoryContextAllocZero(
        SESS_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_EXECUTOR), 1 * sizeof(Port));
     ...
     */
    int retval =
        RAND_priv_bytes((unsigned char*)port->md5Salt, sizeof(port->md5Salt));
    if (retval != 1) {
        ereport(ERROR,
                (errmsg("Failed to Generate the random number,errcode:%d",
                        retval)));
    }
...
void ConnFree(void* conn) {
    int at_main_thread =
        (!IsPostmasterEnvironment || t_thrd.proc_cxt.MyProcPid == PostmasterPid)
            ? 1
            : 0;
...
void ClosePostmasterPorts(bool am_syslogger) {
    ...
    if (close(t_thrd.postmaster_cxt.postmaster_alive_fds[POSTMASTER_FD_OWN]))
        ereport(FATAL,
                (errcode_for_file_access(),
                 errmsg_internal("could not close postmaster death monitoring "
                                 "pipe in child process: %m")));
...
void socket_close_on_exec(void) {
    ...
    for (int i = 0; i < MAXLISTEN; i++) {
        if (t_thrd.postmaster_cxt.ListenSocket[i] != PGINVALID_SOCKET) {
            ...
            if (flags < 0)
                ereport(ERROR, (errcode(ERRCODE_SYSTEM_ERROR),
                                errmsg("fcntl F_GETFD failed!")));

            flags |= FD_CLOEXEC;
            if (fcntl(t_thrd.postmaster_cxt.ListenSocket[i], F_SETFD, flags) <
                0)
                ereport(ERROR, (errcode(ERRCODE_SYSTEM_ERROR),
                                errmsg("fcntl F_SETFD failed!")));
        }
    }
}
...
static void SIGHUP_handler(SIGNAL_ARGS) {
    ...
    if (g_instance.status <= SmartShutdown &&
        ...
        int rc = snprintf_s(gucconf_lock_file, sizeof(gucconf_lock_file),
                            MAXPGPATH - 1, "%s/postgresql.conf.lock",
                            t_thrd.proc_cxt.DataDir);
        securec_check_intval(rc, , );
        if (get_file_lock(gucconf_lock_file, &filelock) != CODE_OK) {
            ereport(WARNING, (errmsg("the last sigup signal is processing,get "
                                     "file lock failed.")));
        ...
        if (g_instance.pid_cxt.PageWriterPID != NULL) {
            int i;
            for (i = 0; i < g_instance.attr.attr_storage.pagewriter_thread_num;
                 i++) {
                if (g_instance.pid_cxt.PageWriterPID[i] != 0) {
                    Assert(!dummyStandbyMode);
        ...
        if (g_instance.pid_cxt.CommReceiverPIDS != NULL) {
            int recv_loop = 0;
            for (recv_loop = 0;
                 recv_loop < g_instance.attr.attr_network.comm_max_receiver;
                 recv_loop++) {
                if (g_instance.pid_cxt.CommReceiverPIDS[recv_loop] != 0) {
                    signal_child(g_instance.pid_cxt.CommReceiverPIDS[recv_loop],
                                 SIGHUP);
                }
            }
        }
...
static void pmdie(SIGNAL_ARGS) {
    ...
    switch (postgres_signal_arg) {
        ...
        case SIGINT:
            ...
            if (STANDBY_MODE == t_thrd.postmaster_cxt.HaShmData->current_mode &&
            ...
            if (g_instance.pid_cxt.CommReceiverPIDS != NULL) {
                ...
                for (recv_loop = 0;
                    ...
                    if (g_instance.pid_cxt.CommReceiverPIDS[recv_loop] != 0) {
                        signal_child(
                            g_instance.pid_cxt.CommReceiverPIDS[recv_loop],
                            SIGTERM);
                    }
                }
            }
            ...
            if (g_instance.pid_cxt.TsCompactionAuxiliaryPID != 0) {
                signal_child(g_instance.pid_cxt.TsCompactionAuxiliaryPID,
                             SIGTERM);
            }
            ...
            if (ENABLE_THREAD_POOL &&
                (pmState == PM_RECOVERY || pmState == PM_STARTUP)) {
                /*
            ...
            if (pmState == PM_RECOVERY) {
                ...
                (void)SignalSomeChildren(
                    SIGTERM, BACKEND_TYPE_NORMAL | BACKEND_TYPE_AUTOVAC);
...
static void SetWalsndsNodeState(ClusterNodeState requester,
    ...
    for (i = 0; i < g_instance.attr.attr_storage.max_wal_senders; i++) {
        ...
        if (walsnd->node_state >= NODESTATE_SMART_DEMOTE_REQUEST &&
            walsnd->node_state <= NODESTATE_FAST_DEMOTE_REQUEST)
...
static void ProcessDemoteRequest(void) {
    ...
    if (pmState != PM_RUN && pmState != PM_HOT_STANDBY &&
        NoDemote == g_instance.demotion) {
        SetWalsndsNodeState(NODESTATE_NORMAL, NODESTATE_NORMAL);
        t_thrd.walsender_cxt.WalSndCtl->demotion = NoDemote;
        ereport(NOTICE, (errmsg("postmaster state not in PM_RUN or "
                                "PM_HOT_STANDBY, it would not demote.")));
        return;
    }

    PMUpdateDBState(DEMOTING_STATE, get_cur_mode(), get_cur_repl_num());
    ereport(LOG, (errmsg("update gaussdb state file: db state(DEMOTING_STATE), "
                         "server mode(%s)",
        ...
        case SmartDemote:
            ...
            if (pmState == PM_RUN || pmState == PM_RECOVERY ||
                ...
                if ((IS_PGXC_COORDINATOR) &&
                    ...
                    signal_child(g_instance.pid_cxt.CommPoolerCleanPID,
                                 SIGTERM);
                ...
                if (g_instance.pid_cxt.TwoPhaseCleanerPID != 0)
                    signal_child(g_instance.pid_cxt.TwoPhaseCleanerPID,
                                 SIGTERM);
                ...
                if (g_instance.pid_cxt.WalWriterAuxiliaryPID != 0)
                    signal_child(g_instance.pid_cxt.WalWriterAuxiliaryPID,
                                 SIGTERM);
                 ...
                 */
                pmState =
                    (pmState == PM_RUN) ? PM_WAIT_BACKUP : PM_WAIT_READONLY;
            }
...
#define LOOPHEADER() (exitstatus = (long)(intptr_t)status)
    ...
    for (;;) {
        ...
        oldpid = pid;
        if (gs_thread_join(t_thrd.postmaster_cxt.CurExitThread,
                           (void**)&status) != 0) {
            /*
            ...
            if (ESRCH == pthread_kill(pid, 0)) {
                exitstatus = 0;
                ereport(LOG,
                        (errmsg("failed to join thread %lu, no such process",
                                pid)));
            } else {
                exitstatus = 1;
            ...
            LOOPHEADER();
            ereport(DEBUG1, (errmsg("have joined thread %lu, exitstatus=%ld.",
                                    pid, exitstatus)));
        }
        ...
        if (pid == g_instance.pid_cxt.StartupPID) {
             ...
             */
            if (g_instance.attr.attr_storage.max_wal_senders > 0 &&
                CountChildren(BACKEND_TYPE_WALSND) > 0) {
                ereport(
                    LOG,
                    (errmsg(
                        "terminating all walsender processes to force cascaded "
            ...
            // add by singheart
            if (g_instance.pid_cxt.NeuTransactionSenderPIDS == 0) {
                g_instance.pid_cxt.NeuTransactionSenderPIDS = initialize_util_thread(NEU_TRANSACTION_SENDER);
            }
            if (g_instance.pid_cxt.NeuResponseReceiverPIDS == 0) {
                g_instance.pid_cxt.NeuResponseReceiverPIDS = initialize_util_thread(NEU_RESPONSE_RECEIVER);
            }
            if (g_instance.pid_cxt.NeuApplyLogReceiverPIDS == 0) {
                g_instance.pid_cxt.NeuApplyLogReceiverPIDS = initialize_util_thread(NEU_APPLY_LOG_RECEIVER);
            }


            if (g_instance.pid_cxt.WalWriterPID == 0)
                g_instance.pid_cxt.WalWriterPID =
                    initialize_util_thread(WALWRITER);
             ...
             */
            if (!u_sess->proc_cxt.IsBinaryUpgrade && AutoVacuumingActive() &&
                g_instance.pid_cxt.AutoVacPID == 0 && !dummyStandbyMode &&
                u_sess->attr.attr_common.upgrade_mode != 1 &&
                !InplaceUpgradePrecommit)
                g_instance.pid_cxt.AutoVacPID =
                    initialize_util_thread(AUTOVACUUM_LAUNCHER);
            ...
            if (ENABLE_ASP && g_instance.pid_cxt.AshPID == 0 &&
                !dummyStandbyMode)
                g_instance.pid_cxt.AshPID = initialize_util_thread(ASH_WORKER);
            ...
            if (g_instance.pid_cxt.PgAuditPID == 0 &&
                u_sess->attr.attr_security.Audit_enabled && !dummyStandbyMode)
                g_instance.pid_cxt.PgAuditPID = pgaudit_start();

            if (t_thrd.postmaster_cxt.audit_primary_start &&
            ...
            /* if workload manager is off, we still use this thread to build user hash table */
            if ((ENABLE_WORKLOAD_CONTROL || !WLMIsInfoInit()) &&
                g_instance.pid_cxt.WLMCollectPID == 0 && !dummyStandbyMode) {
            ...
            if (ENABLE_WORKLOAD_CONTROL &&
                (g_instance.pid_cxt.WLMMonitorPID == 0) && !dummyStandbyMode)
                g_instance.pid_cxt.WLMMonitorPID =
                    initialize_util_thread(WLM_MONITOR);

            if (ENABLE_WORKLOAD_CONTROL &&
                (g_instance.pid_cxt.WLMArbiterPID == 0) && !dummyStandbyMode)
                g_instance.pid_cxt.WLMArbiterPID =
                    initialize_util_thread(WLM_ARBITER);
            ...
            if (g_instance.pid_cxt.RemoteServicePID == 0 && !dummyStandbyMode &&
                IS_PGXC_DATANODE && t_thrd.postmaster_cxt.HaShmData &&
                t_thrd.postmaster_cxt.HaShmData->current_mode != NORMAL_MODE &&
                !IS_DN_WITHOUT_STANDBYS_MODE() && IsRemoteReadModeOn())
                g_instance.pid_cxt.RemoteServicePID =
                    initialize_util_thread(RPC_SERVICE);
            ...
            if (NeedHeartbeat())
                g_instance.pid_cxt.HeartbeatPID =
                    initialize_util_thread(HEARTBEAT);
            ...
            ereport(LOG, (errmsg("update gaussdb state file: db "
                                 "state(NORMAL_STATE), server mode(%s)",
                                 wal_get_role_string(get_cur_mode()))));
            ...
            for (i = 0; i < thread_num; i++) {
                if (pid == g_instance.pid_cxt.CkptBgWriterPID[i]) {
                    ...
                    if (!EXIT_STATUS_0(exitstatus)) {
                        HandleChildCrash(
                            pid, exitstatus,
                            _("incre ckpt background writer process"));
                    }
                    continue;
                }
            }
        ...
        if (pid == g_instance.pid_cxt.CheckpointerPID) {
            ...
            if (EXIT_STATUS_0(exitstatus) && pmState == PM_SHUTDOWN) {
                 ...
                 */
                Assert(g_instance.status > NoShutdown ||
                       g_instance.demotion > NoDemote);
        ...
        if (pid == g_instance.pid_cxt.PgStatPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("statistics collector process"), pid,
                             exitstatus);
        ...
        if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) &&
            pid == g_instance.pid_cxt.SnapshotPID) {
            Assert(!dummyStandbyMode);
            g_instance.pid_cxt.SnapshotPID = 0;

            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("snapshot collector process"), pid,
                             exitstatus);
        ...
        if (pid == g_instance.pid_cxt.AshPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("Active session history collector process"),
                             pid, exitstatus);
        ...
        if (pid == g_instance.pid_cxt.StatementPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("full SQL statement flush process"), pid,
                             exitstatus);
        ...
        if ((IS_PGXC_COORDINATOR || IS_SINGLE_NODE) &&
            pid == g_instance.pid_cxt.PercentilePID) {
            Assert(!dummyStandbyMode);
            g_instance.pid_cxt.PercentilePID = 0;

            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("percentile collector process"), pid,
                             exitstatus);
        ...
        if (pid == g_instance.pid_cxt.CPMonitorPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("compute pool monitor process"), pid,
                             exitstatus);
        ...
        if (START_BARRIER_CREATOR &&
            pid == g_instance.pid_cxt.BarrierCreatorPID) {
            g_instance.pid_cxt.BarrierCreatorPID = 0;
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("barrier creator process"), pid,
                             exitstatus);
            continue;
        }
        ...
        if (get_real_recovery_parallelism() > 1) {
            ...
            if (pageredoStatus == PAGE_REDO_THREAD_EXIT_NORMAL) {
                ...
                ereport(LOG,
                        (errmsg("aborting  due to page redo process failure")));
        ...
        if (pid == g_instance.pid_cxt.TsCompactionPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                HandleChildCrash(pid, exitstatus,
                                 _("timeseries compaction process"));
            continue;
        }
        ...
        if (pid == g_instance.pid_cxt.TsCompactionAuxiliaryPID) {
            ...
            if (!EXIT_STATUS_0(exitstatus))
                HandleChildCrash(pid, exitstatus,
                                 _("timeseries compaction auxiliary process"));
            continue;
        }

        if (streaming_backend_manager(STREAMING_BACKEND_REAP, (void*)&pid)) {
            if (!EXIT_STATUS_0(exitstatus))
                LogChildExit(LOG, _("streaming backend process"), pid,
                             exitstatus);
            continue;
        }
#endif /* ENABLE_MULTIPLE_NODES */
 ...
 */
static void CleanupBackend(ThreadId pid,
                           int exitstatus) /* child's exit status. */
{
     ...
     */
    for (curr = DLGetTail(g_instance.backend_list); curr;
         curr = DLGetPred(curr)) {
...
void HandleChildCrash(ThreadId pid, int exitstatus, const char* procname) {
    ...
    ereport(LOG, (errmsg("%s (ThreadId %lu) exited with exit code %d", procname,
    ...
    if (WIFEXITED(exitstatus)) {
        ereport(
            ...
            /* ------
              ...
              "server process" */
            (errmsg("%s (ThreadId %lu) exited with exit code %d", procname, pid,
                    WEXITSTATUS(exitstatus)),
             activity ? errdetail("Failed process was running: %s", activity)
                      : 0));
        ...
        ereport(
            ...
            /* ------
              translator: %s is a noun phrase describing a child process, such as
              "server process" */
            (errmsg("%s (ThreadId %lu) was terminated by exception 0x%X",
                    procname, pid, WTERMSIG(exitstatus)),
             errhint("See C include file \"ntstatus.h\" for a description of "
        ...
        ereport(
            ...
            /* ------
              translator: %s is a noun phrase describing a child process, such as
              "server process" */
            (errmsg("%s (ThreadId %lu) was terminated by signal %d", procname,
        ...
        ereport(
            ...
            /* ------
              translator: %s is a noun phrase describing a child process, such as
              "server process" */
            (errmsg("%s (ThreadId %lu) exited with unrecognized status %d",
...
static bool ckpt_all_flush_buffer_thread_exit() {
    if (g_instance.pid_cxt.PageWriterPID[0] == 0 &&
        (g_instance.pid_cxt.CkptBgWriterPID == NULL ||
         g_instance.pid_cxt.CkptBgWriterPID[0] == 0)) {
        return true;
    } else {
        return false;
    }
...
static void PostmasterStateMachine(void) {
            ...
            if (g_instance.fatal_error) {
                ...
                if (g_instance.pid_cxt.PageWriterPID != NULL) {
                    ...
                    for (int i = 0;
                         ...
                         i++) {
                        if (g_instance.pid_cxt.PageWriterPID[i] != 0) {
                            signal_child(g_instance.pid_cxt.PageWriterPID[i],
                                         SIGTERM);
                        }
                    }
                }
                ...
                if (g_instance.pid_cxt.CkptBgWriterPID != NULL) {
                    ...
                    for (int i = 0; i < thread_num; i++) {
                        if (g_instance.pid_cxt.CkptBgWriterPID[i] != 0) {
                            signal_child(g_instance.pid_cxt.CkptBgWriterPID[i],
                                         SIGTERM);
                        }
                    }
                }
    ...
    if (pmState == PM_WAIT_DEAD_END) {
         ...
         */
        if (DLGetHead(g_instance.backend_list) == NULL &&
            g_instance.pid_cxt.PgArchPID == 0 &&
            g_instance.pid_cxt.PgStatPID == 0 &&
            g_instance.pid_cxt.PgAuditPID == 0 &&
    ...
    if (g_instance.fatal_error && pmState == PM_NO_CHILDREN) {
        ...
        if (ENABLE_GPC) {
            ...
            if (g_threadPoolControler &&
                g_threadPoolControler->GetScheduler()->HasShutDown() == false)
                g_threadPoolControler->ShutDownScheduler(true);
        }
        shmem_exit(1);
        reset_shared(g_instance.attr.attr_network.PostPortNumber);
    ...
    if (pmState == PM_STARTUP && t_thrd.xlog_cxt.server_mode == STANDBY_MODE &&
...
void signalBackend(Backend* bn, int signal, int be_mode) {
    if ((uint32)bn->flag & THRD_EXIT)
        ereport(LOG,
                (errmsg("Thread pid(%lu), flag(%d) may be exited repeatedly",
                        bn->pid, bn->flag)));
    ...
    if (gs_signal_send(bn->pid, signal, nowait) != 0) {
        ereport(WARNING,
                (errmsg("kill(pid %ld, signal %d) failed,"
                        " thread name \"%s\", pmState %d, Demotion %d, "
                        "Shutdown %d, backend type %d",
                        (long)bn->pid, signal, GetProcName(bn->pid), pmState,
                        g_instance.demotion, Shutdown, be_mode)));
    }
}
    ...
    if (0 != (err = gs_signal_send(pid, signal))) {
        ereport(WARNING,
                (errmsg("kill(pid %ld, signal %d) failed: \"%s\","
                        " thread name \"%s\", pmState %d, Demotion %d, "
                        "Shutdown %d, backend type %d",
                        (long)pid, signal, gs_strerror(err), GetProcName(pid),
                        pmState, g_instance.demotion, Shutdown, be_mode)));
    }
}
...
static bool SignalSomeChildren(int signal, int target) {
    ...
    for (curr = DLGetHead(g_instance.backend_list); curr;
         curr = DLGetSucc(curr)) {
        Backend* bp = (Backend*)DLE_VAL(curr);
        ...
        ereport(DEBUG4, (errmsg_internal("sending signal %d to process %lu",
                                         signal, bp->pid)));
...
static int IsHaWhiteListIp(const Port* port) {
    ...
    if (IS_PGXC_COORDINATOR &&
        ...
        ereport(
            WARNING,
            (errmsg(
                "the ha listen ip and port is not for remote client from [%s].",
                ipstr)));
        return STATUS_ERROR;
    }
...
static int BackendStartup(Port* port, bool isConnectHaPort) {
    ...
    if (port->canAcceptConnections != CAC_OK &&
        port->canAcceptConnections != CAC_WAITBACKUP) {
        (void)ReleasePostmasterChildSlot(childSlot);
        if (port->canAcceptConnections == CAC_TOOMANY)
            ereport(WARNING,
                    (errmsg("could not fork new process for connection due to "
                            "too many connections")));
        else
            ereport(WARNING, (errmsg("could not fork new process for "
                                     "connection due to PMstate %s",
                                     GetPMState(pmState))));
    ...
    if (CAC_SHUTDOWN == port->canAcceptConnections) {
        (void)ReleasePostmasterChildSlot(childSlot);
        ereport(WARNING, (errmsg("could not fork new process for "
                                 "connection:postmaster is shutting down")));
        report_fork_failure_to_client(port, 0,
                                      "could not fork new process for "
                                      "connection:postmaster is shutting down");
        return STATUS_ERROR;
    }
    ...
    if (pid == (ThreadId)-1) {
        ...
        ereport(
            WARNING,
            (errmsg("could not fork new process for connection socket %d : %m",
                    (int)port->sock)));
    ...
    ereport(DEBUG2, (errmsg_internal("forked new backend, pid=%lu socket=%d",
                                     pid, (int)port->sock)));
...
static void report_fork_failure_to_client(Port* port, int errnum,
    ...
    if (specialErrorInfo == NULL) {
        ...
        ret = sprintf_s(buffer, sizeof(buffer), "E%s%s\n",
        ...
        if (len >= sizeof(buffer)) {
            rc = snprintf_truncated_s(buffer, sizeof(buffer), "%s",
                                      specialErrorInfo);
...
int ClientConnInitilize(Port* port) {
...
#ifdef ENABLE_MULTIPLE_NODES
    if (!enable_sig_alarm(
            (u_sess->attr.attr_network.PoolerConnectTimeout - 1) * 1000,
            false)) {
        ereport(FATAL,
                (errmsg("could not set timer for startup packet timeout")));
    }
#endif
    ...
    if (!disable_sig_alarm(false))
        ereport(FATAL,
                (errmsg("could not disable timer for startup packet timeout")));
...
static void RemoteHostInitilize(Port* port) {
    ...
    if (pg_getnameinfo_all(
            ...
            (u_sess->attr.attr_common.log_hostname ? 0 : NI_NUMERICHOST) |
                NI_NUMERICSERV) != 0) {
     ...
     */
    port->remote_host = MemoryContextStrdup(
        SESS_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_EXECUTOR), remote_host);
    port->remote_port = MemoryContextStrdup(
        SESS_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_EXECUTOR), remote_port);
...
static void PsDisplayInitialize(Port* port) {
    ...
    if (port->remote_port[0] == '\0')
        rc = snprintf_s(remote_ps_data, sizeof(remote_ps_data), NI_MAXHOST,
                        "%s", port->remote_host);
    else
        rc = snprintf_s(remote_ps_data, sizeof(remote_ps_data),
                        sizeof(remote_ps_data) - 1, "%s(%s)", port->remote_host,
                        port->remote_port);
...
void CheckClientIp(Port* port) {
    ...
    if (!check_ip_whitelist(port, ip, IP_LEN)) {
        ...
        t_thrd.postgres_cxt.whereToSendOutput =
            DestRemote; /* now safe to ereport to client */
...
static void handle_recovery_started() {
    if (pmState == PM_STARTUP && g_instance.status == NoShutdown &&
        !dummyStandbyMode) {
        ...
        if (g_instance.attr.attr_storage.enableIncrementalCheckpoint) {
            for (int i = 0;
                 i < g_instance.attr.attr_storage.pagewriter_thread_num; i++) {
...
static void handle_begin_hot_standby() {
    if ((dummyStandbyMode || pmState == PM_RECOVERY) &&
...
static void handle_promote_signal() {
    if (g_instance.pid_cxt.StartupPID != 0 &&
        ...
        if (GetHaShmemMode() != STANDBY_MODE) {
            ereport(
                LOG,
                (errmsg("Instance can't be promoted in none standby mode.")));
        } else {
            /* Database Security: Support database audit */
            if (t_thrd.walreceiverfuncs_cxt.WalRcv &&
                ...
                SendNotifySignal(NOTIFY_SWITCHOVER,
                                 g_instance.pid_cxt.StartupPID);
            } else {
                if (t_thrd.walreceiverfuncs_cxt.WalRcv)
                    t_thrd.walreceiverfuncs_cxt.WalRcv->node_state =
                ...
                SendNotifySignal(NOTIFY_FAILOVER,
                                 g_instance.pid_cxt.StartupPID);
            }
        }
    }
...
static void handle_primary_signal(volatile HaShmemData* hashmdata) {
    ...
    if (hashmdata->current_mode == NORMAL_MODE) {
        if (g_instance.attr.attr_storage.max_wal_senders < 2 ||
            ...
            ereport(WARNING,
                    (errmsg("when notifying normal mode to primary mode, \
    ...
    PMUpdateDBState(NORMAL_STATE, get_cur_mode(), get_cur_repl_num());
    ereport(LOG, (errmsg("update gaussdb state file: db state(NORMAL_STATE), "
                         "server mode(%s)",
                         wal_get_role_string(get_cur_mode()))));
}
...
static void handle_standby_signal(volatile HaShmemData* hashmdata) {
    if (g_instance.pid_cxt.StartupPID != 0 &&
        ...
        ereport(LOG, (errmsg("update gaussdb state file: db "
                             "state(NEEDREPAIR_STATE), server mode(%s)",
                             wal_get_role_string(get_cur_mode()))));
        /*
...
static void sigusr1_handler(SIGNAL_ARGS) {
    ...
    if (CheckPostmasterSignal(PMSIGNAL_LOCAL_RECOVERY_DONE) &&
        ...
        PMUpdateDBStateLSN();
        ereport(LOG,
                (errmsg("set lsn after recovery done in gaussdb state file")));
    }
    ...
    if (g_instance.pid_cxt.WalWriterAuxiliaryPID == 0 &&
        ...
        g_instance.pid_cxt.WalWriterAuxiliaryPID =
            initialize_util_thread(WALWRITERAUXILIARY);
        ereport(LOG,
                (errmsg("sigusr1_handler create WalWriterAuxiliary(%lu) after "
                        "local recovery is done. pmState:%u, ServerMode:%u",
                        g_instance.pid_cxt.WalWriterAuxiliaryPID, pmState,
                        t_thrd.postmaster_cxt.HaShmData->current_mode)));
    }
    ...
    if (CheckPostmasterSignal(PMSIGNAL_UPDATE_WAITING)) {
        ...
        ereport(LOG, (errmsg("set gaussdb state file: db state(WAITING_STATE), "
                             "server mode(%s)",
                             wal_get_role_string(get_cur_mode()))));
    }
    if (CheckPostmasterSignal(PMSIGNAL_UPDATE_PROMOTING)) {
        PMUpdateDBState(PROMOTING_STATE, get_cur_mode(), get_cur_repl_num());
        ereport(LOG, (errmsg("set gaussdb state file: db "
                             "state(PROMOTING_STATE), server mode(%s)",
                             wal_get_role_string(get_cur_mode()))));
    ...
    /* should not start a worker in shutdown or demotion procedure */
    if (CheckPostmasterSignal(PMSIGNAL_START_CLEAN_STATEMENT) &&
        g_instance.status == NoShutdown && g_instance.demotion == NoDemote) {
    ...
    if (CheckPostmasterSignal(PMSIGNAL_START_WALRECEIVER) &&
        ...
        g_instance.status == NoShutdown) {
        if (g_instance.pid_cxt.WalRcvWriterPID == 0) {
            g_instance.pid_cxt.WalRcvWriterPID =
                initialize_util_thread(WALRECWRITE);
    ...
    if (CheckPostmasterSignal(PMSIGNAL_START_DATARECEIVER) &&
        g_instance.pid_cxt.DataReceiverPID == 0 &&
        (pmState == PM_STARTUP || pmState == PM_RECOVERY ||
         pmState == PM_HOT_STANDBY || pmState == PM_WAIT_READONLY) &&
        g_instance.status == NoShutdown) {
        if (g_instance.pid_cxt.DataRcvWriterPID == 0) {
            g_instance.pid_cxt.DataRcvWriterPID =
                initialize_util_thread(DATARECWRITER);
            SetDataRcvWriterPID(g_instance.pid_cxt.DataRcvWriterPID);
        }

        /* Startup Process wants us to start the datareceiver process. */
        g_instance.pid_cxt.DataReceiverPID =
            initialize_util_thread(DATARECIVER);
    }

    if (CheckPostmasterSignal(PMSIGNAL_START_CATCHUP) &&
        g_instance.pid_cxt.CatchupPID == 0 && g_instance.status == NoShutdown) {
    ...
    if ((mode = CheckSwitchoverSignal()) != 0 && WalRcvIsOnline() &&
        DataRcvIsOnline() &&
        (pmState == PM_STARTUP || pmState == PM_RECOVERY ||
         pmState == PM_HOT_STANDBY || pmState == PM_WAIT_READONLY)) {
    ...
    if (CheckFinishRedoSignal() &&
        g_instance.comm_cxt.localinfo_cxt.is_finish_redo != 1) {
        pg_atomic_write_u32(&(g_instance.comm_cxt.localinfo_cxt.is_finish_redo),
                            1);
    }
    ...
    if (CheckPostmasterSignal(PMSIGNAL_ROLLBACK_STANDBY_PROMOTE)) {
        PMUpdateDBState(NORMAL_STATE, get_cur_mode(), get_cur_repl_num());
        ereport(LOG, (errmsg("update gaussdb state file:"
                             "db state(NORMAL_STATE), server mode(%s), "
                             "reason(switchover failure)",
                             wal_get_role_string(get_cur_mode()))));
    }
...
void ReaperBackendMain() {
    ...
    for (;;) {
         ...
         */
        (void)WaitLatch(&t_thrd.postmaster_cxt.ReaperBackendLatch,
                        WL_LATCH_SET | WL_TIMEOUT, 10 * 1000);
    }
...
#define OPTS_TEMP_FILE "postmaster.opts.tmp"
    ...
    if ((fp = fopen(OPTS_TEMP_FILE, "w")) == NULL) {
        ereport(LOG,
                (errmsg("could not create file \"%s\": %m", OPTS_TEMP_FILE)));
        return;
    }
    ...
    if (fclose(fp)) {
        ereport(LOG,
                (errmsg("could not close file \"%s\": %m", OPTS_TEMP_FILE)));
        return;
    }
...
#endif
{
    ...
    ss_rc = memcpy_s(&param->ListenSocket, sizeof(param->ListenSocket),
    ...
    int rc =
        memcpy_s(param->LocalAddrList, (MAXLISTEN * IP_LEN),
                 t_thrd.postmaster_cxt.LocalAddrList, (MAXLISTEN * IP_LEN));
    securec_check(rc, "", "");
    ...
    ss_rc = memcpy_s(&param->syslogPipe, sizeof(param->syslogPipe),
...
static bool write_duplicated_handle(HANDLE* dest, HANDLE src,
    ...
    if (!DuplicateHandle(GetCurrentProcess(), src, childProcess, &hChild, 0,
                         TRUE,
                         DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)) {
        ereport(LOG,
                (errmsg_internal("could not duplicate handle to be written to "
...
static bool write_inheritable_socket(InheritableSocket* dest, SOCKET src,
    ...
    if (src != 0 && src != PGINVALID_SOCKET) {
        ...
        if (WSADuplicateSocket(src, childpid, &dest->wsainfo) != 0) {
            ereport(LOG, (errmsg("could not duplicate socket %d for use in "
                                 "backend: error code %d",
    ...
    if (src->origsocket == PGINVALID_SOCKET || src->origsocket == 0) {
        ...
        *dest = src->origsocket;
        ...
        s = WSASocket(FROM_PROTOCOL_INFO, FROM_PROTOCOL_INFO,
                      FROM_PROTOCOL_INFO, &src->wsainfo, 0, 0);
...
static void restore_backend_variables(BackendParameters* param, Port* port) {
    ...
    t_thrd.shemem_ptr_cxt.mainLWLockArray =
        (LWLockPadded*)param->mainLWLockArray;
    t_thrd.shemem_ptr_cxt.PMSignalState = param->PMSignalState;

    t_thrd.postmaster_cxt.LocalIpNum = param->LocalIpNum;
    rc = memcpy_s(t_thrd.postmaster_cxt.LocalAddrList, (MAXLISTEN * IP_LEN),
                  param->LocalAddrList, (MAXLISTEN * IP_LEN));
    securec_check(rc, "", "");
...
#endif
    ss_rc =
        memcpy_s(&t_thrd.postmaster_cxt.syslogPipe,
                 sizeof(t_thrd.postmaster_cxt.syslogPipe), &param->syslogPipe,
                 sizeof(t_thrd.postmaster_cxt.syslogPipe));
    securec_check(ss_rc, "\0", "\0");
...
static void InitPostmasterDeathWatchHandle(void) {
     ...
     * call.
     */
    if (fcntl(t_thrd.postmaster_cxt.postmaster_alive_fds[POSTMASTER_FD_WATCH],
              F_SETFL, O_NONBLOCK))
        ereport(FATAL,
                (errcode_for_socket_access(),
                 errmsg_internal("could not set postmaster death monitoring "
                                 "pipe to non-blocking mode: %m")));
...
void CBMShmemInit(void) {
    ...
    t_thrd.cbm_cxt.XlogCbmSys =
        (XlogBitmap*)ShmemInitStruct("CBM Shmem Data", CBMShmemSize(), &found);
...
void HaShmemInit(void) {
    ...
    t_thrd.postmaster_cxt.HaShmData =
        (HaShmemData*)ShmemInitStruct("HA Shmem Data ", HaShmemSize(), &found);
...
static bool IsChannelAdapt(Port* port, ReplConnInfo* repl) {
    ...
    if (AF_INET6 == laddr->sa_family) {
        result =
            inet_net_ntop(AF_INET6, &((struct sockaddr_in*)laddr)->sin_addr,
                          128, local_ip, IP_LEN);
        if (NULL == result) {
            ereport(WARNING,
                    (errmsg("inet_net_ntop failed, error: %d", EAFNOSUPPORT)));
        }
    } else if (AF_INET == laddr->sa_family) {
        result = inet_net_ntop(AF_INET, &((struct sockaddr_in*)laddr)->sin_addr,
                               32, local_ip, IP_LEN);
        if (NULL == result) {
            ereport(WARNING,
                    (errmsg("inet_net_ntop failed, error: %d", EAFNOSUPPORT)));
    ...
    if (AF_INET6 == raddr->sa_family) {
        result =
            inet_net_ntop(AF_INET6, &((struct sockaddr_in*)raddr)->sin_addr,
                          128, remote_ip, IP_LEN);
        if (NULL == result) {
            ereport(WARNING,
                    (errmsg("inet_net_ntop failed, error: %d", EAFNOSUPPORT)));
        }
    } else if (AF_INET == raddr->sa_family) {
        result = inet_net_ntop(AF_INET, &((struct sockaddr_in*)raddr)->sin_addr,
                               32, remote_ip, IP_LEN);
        if (NULL == result) {
            ereport(WARNING,
                    (errmsg("inet_net_ntop failed, error: %d", EAFNOSUPPORT)));
    ...
    if (0 == strcmp(local_ip, repl->localhost) &&
        0 == strcmp(remote_ip, repl->remotehost)) {
        return true;
    } else {
        ereport(DEBUG1, (errmsg("connect local ip %s, connect remote ip %s",
                                local_ip, remote_ip)));
        return false;
    }
}
...
bool IsFromLocalAddr(Port* port) {
    ...
    if (AF_INET6 == local_addr->sa_family) {
        ...
        result =
            inet_net_ntop(AF_INET, &((struct sockaddr_in*)local_addr)->sin_addr,
                          32, local_ip, IP_LEN);
    }

    if (result == NULL) {
        ereport(WARNING, (errmsg("inet_net_ntop local failed, error: %d",
                                 EAFNOSUPPORT)));
    }
    ...
    if (result == NULL) {
        ereport(WARNING, (errmsg("inet_net_ntop remote failed, error: %d",
                                 EAFNOSUPPORT)));
    }

    /* only the remote ip equal to local ip, return true */
    if (strlen(local_ip) != 0 && strlen(remote_ip) != 0 &&
        strcmp(local_ip, remote_ip) == 0)
        return true;
    else
        return false;
}
...
bool IsLocalAddr(Port* port) {
    ...
    for (i = 0; i != t_thrd.postmaster_cxt.LocalIpNum; ++i) {
        if (0 == strcmp(local_ip, t_thrd.postmaster_cxt.LocalAddrList[i]) ||
            (AF_INET == laddr->sa_family &&
             0 == strcmp("0.0.0.0", t_thrd.postmaster_cxt.LocalAddrList[i])) ||
            (AF_INET6 == laddr->sa_family &&
             0 == strcmp("::", t_thrd.postmaster_cxt.LocalAddrList[i]))) {
 ...
 */
static bool IsLocalIp(const char* address) {
    return (strcmp(address, LOCAL_HOST) == 0 ||
            strcmp(address, LOOP_IP_STRING) == 0 ||
            strcmp(address, LOOP_IPV6_IP) == 0);
}
    ...
    for (int i = 1; i < MAX_REPLNODE_NUM; i++) {
        if (t_thrd.postmaster_cxt.ReplConnArray[i] != NULL &&
            t_thrd.postmaster_cxt.ReplConnArray[i]->localport ==
                g_instance.attr.attr_network.PoolerPort &&
            (strcmp(t_thrd.postmaster_cxt.ReplConnArray[i]->localhost, "*") ==
                 0 ||
             strcmp(t_thrd.postmaster_cxt.ReplConnArray[i]->localhost,
                    hostName) == 0)) {
            index = i;
...
bool IsHASocketAddr(struct sockaddr* sock_addr) {
    ...
    switch (sock_addr->sa_family) {
        ...
        case AF_UNIX:
            ...
            UNIXSOCK_PATH(unixStr, g_instance.attr.attr_network.PoolerPort,
                          g_instance.attr.attr_network.UnixSocketDir);

            if (0 ==
                (strncmp(unixaddr->sun_path, unixStr, MAX_UNIX_PATH_LEN))) {
                result = true;
            }
            break;
#endif
...
static bool IsAlreadyListen(const char* ip, int port) {
    ...
    for (listen_index = 0; listen_index != MAXLISTEN; ++listen_index) {
        if (t_thrd.postmaster_cxt.ListenSocket[listen_index] !=
            PGINVALID_SOCKET) {
            ...
            if (getsockname(t_thrd.postmaster_cxt.ListenSocket[listen_index],
                            (struct sockaddr*)&saddr, (socklen_t*)&slen) < 0) {
                ereport(LOG,
                        (errmsg("Error in getsockname int IsAlreadyListen()")));
                continue;
            }

            if (AF_INET6 == saddr.sin_family) {
                result = inet_net_ntop(AF_INET6, &saddr.sin_addr, 128, sock_ip,
                                       IP_LEN);
                if (NULL == result) {
                    ereport(WARNING, (errmsg("inet_net_ntop failed, error: %d",
                                             EAFNOSUPPORT)));
                }
            } else if (AF_INET == saddr.sin_family) {
                result = inet_net_ntop(AF_INET, &saddr.sin_addr, 32, sock_ip,
                                       IP_LEN);
                if (NULL == result) {
                    ereport(WARNING, (errmsg("inet_net_ntop failed, error: %d",
                                             EAFNOSUPPORT)));
...
bool CheckSockAddr(struct sockaddr* sock_addr, const char* szIP, int port) {
    ...
    if ((NULL == sock_addr) || (NULL == szIP) || (0 == port)) {
        ereport(LOG,
                (errmsg("invalid socket information or IP string or port")));
        return false;
    }
    ...
    switch (sock_addr->sa_family) {
        ...
        case AF_INET:
            ipv4addr = (struct sockaddr_in*)sock_addr;
            result = inet_net_ntop(AF_INET, &(ipv4addr->sin_addr), 32, IPstr,
                                   MAX_IP_STR_LEN - 1);
            if (NULL == result) {
                ereport(WARNING, (errmsg("inet_net_ntop failed, error: %d",
                                         EAFNOSUPPORT)));
        ...
        case AF_INET6:
            ipv6addr = (struct sockaddr_in6*)sock_addr;
            result = inet_net_ntop(AF_INET6, &(ipv6addr->sin6_addr), 128, IPstr,
                                   MAX_IP_STR_LEN - 1);
            if (NULL == result) {
                ereport(WARNING, (errmsg("inet_net_ntop failed, error: %d",
                                         EAFNOSUPPORT)));
 ...
 */
void CreateServerSocket(char* ipaddr, int portNumber, int enCreatemode,
                        int* success, bool add_localaddr_flag,
                        bool is_create_psql_sock) {
    int status = 0;
    int successCount = 0;
    ...
    if (((unsigned int)enCreatemode & NEED_CREATE_TCPIP) &&
        (!IsAlreadyListen(ipaddr, portNumber))) {
        ereport(LOG,
                (errmsg("Create TCP/IP socket [%s:%d]", ipaddr, portNumber)));

        if (strcmp(ipaddr, "*") == 0) {
            status = StreamServerPort(
                AF_UNSPEC, NULL, (unsigned short)portNumber,
                g_instance.attr.attr_network.UnixSocketDir,
                t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN,
                add_localaddr_flag, is_create_psql_sock, false);
        } else {
            status = StreamServerPort(
                AF_UNSPEC, ipaddr, (unsigned short)portNumber,
                g_instance.attr.attr_network.UnixSocketDir,
                t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN,
                add_localaddr_flag, is_create_psql_sock, false);
        }
        if (status == STATUS_OK) {
            ...
            ereport(WARNING, (errmsg("could not create listen socket for "
                                     "\"ipaddr:%s,portNumber:%d\"",
                                     ipaddr, portNumber)));
        }
    } else {
        successCount++;
        ereport(LOG, (errmsg("TCP/IP socket [%s:%d] has been created already, "
                             "no need recreate it",
                             ipaddr, portNumber)));
    }
...
bool ParseHaListenAddr(LISTEN_ADDRS* pListenList) {
    ...
    if (NULL == pListenList) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                        errmsg("parameter error in ParseHaListenAddr()")));
        return false;
    }
    ...
    for (i = 1; i < MAX_REPLNODE_NUM; i++) {
        if (t_thrd.postmaster_cxt.ReplConnArray[i] != NULL) {
            pListenList->lsnArray[count].portnum =
                t_thrd.postmaster_cxt.ReplConnArray[i]->localport;
...
static void CreateHaListenSocket(void) {
    ...
    int ss_rc = memset_s(&newListenAddrs, sizeof(newListenAddrs), 0,
                         sizeof(newListenAddrs));
    securec_check(ss_rc, "\0", "\0");

    /* parse the ReplConnArray into IP-Port pair */
    if (!ParseHaListenAddr(&newListenAddrs)) {
        ereport(WARNING,
                (errmsg("\"replconninfo\" has been set null or invalid ")));
    }
    ...
    for (i = 0; i < MAXLISTEN; i++) {
        ...
        if (getsockname(t_thrd.postmaster_cxt.ListenSocket[i],
                        (struct sockaddr*)&sock_addr, (socklen_t*)&s_len) < 0) {
            ereport(LOG,
                    (errmsg("Error in getsockname int IsAlreadyListen()")));
        }
        success = 0;
        ...
        for (j = 0; j < newListenAddrs.usedNum; j++) {
            if (CheckSockAddr((struct sockaddr*)&sock_addr,
                ...
                if (AF_UNIX == ((struct sockaddr*)&sock_addr)->sa_family) {
                    if (newListenAddrs.lsnArray[j].portnum !=
                        g_instance.attr.attr_network.PoolerPort)
                        ereport(WARNING, (errmsg("replication socket could not "
                                                 "be Unix Domain Socket, "
                                                 "something must be wrong of "
                                                 "the Ha listen socket")));
                } else {
                    newListenAddrs.lsnArray[j].createmodel =
                        (uint32)newListenAddrs.lsnArray[j].createmodel &
                        ~NEED_CREATE_TCPIP;
                }
                success++;
            }
    ...
    if (0 == success) {
        ereport(LOG, (errmsg("could not create any TCP/IP sockets for HA "
                             "listen addresses")));
    } else {
        ereport(LOG, (errmsg("number of valid HA TCP/IP sockets is \"%d\"",
                             success)));
    }
 ...
 */
static void ListenSocketRegulation(void) {
    IntArrayRegulation((int*)t_thrd.postmaster_cxt.ListenSocket, MAXLISTEN,
                       (int)PGINVALID_SOCKET);

    IntArrayRegulation((int*)t_thrd.postmaster_cxt.listen_sock_type, MAXLISTEN,
                       UNUSED_LISTEN_SOCKET);
}

DbState get_local_dbstate_sub(WalRcvData* walrcv, ServerMode mode) {
    bool has_build_reason = true;
    if ((t_thrd.postmaster_cxt.HaShmData
                 ->repl_reason[t_thrd.postmaster_cxt.HaShmData->current_repl] ==
             NONE_REBUILD &&
         walrcv != NULL && walrcv->isRuning &&
...
static void PMReadDBStateFile(GaussState* state) {
    ...
    if (NULL == state) {
        ereport(LOG,
                (errmsg("%s: parameter state is null in PMReadDBStateFile()",
                        progname)));
        return;
    }
    ...
    if (NULL == statef) {
        ereport(LOG, (errmsg("%s: open gaussdb state file \"%s\" failed, could "
                             "not read the build infomation: %m",
                             progname, gaussdb_state_file)));
        return;
    }

    if (0 == (fread(state, 1, sizeof(GaussState), statef))) {
        ereport(LOG, (errmsg("%s: read gaussdb state infomation from the file "
                             "\"%s\" failed: %m",
                             progname, gaussdb_state_file)));
    }
    fclose(statef);
...
static void PMSetDBStateFile(GaussState* state) {
    ...
    if (NULL == state) {
        ereport(LOG,
                (errmsg("%s: parameter state is null in PMSetDBStateFile()",
                        progname)));
        return;
    }

    int rc = snprintf_s(temppath, MAXPGPATH, MAXPGPATH - 1, "%s.temp",
                        gaussdb_state_file);
    securec_check_intval(rc, , );
    ...
    if (0 == (fwrite(state, 1, sizeof(GaussState), statef))) {
        ereport(LOG, (errmsg("%s: update gaussdb state infomation from the "
                             "file \"%s\" failed: %m",
                             progname, temppath)));
    }
    ...
    if (fsync(fileno(statef)) == 0) {
        ereport(LOG,
                (errmsg("%s: fsync file \"%s\" success", progname, temppath)));
    } else {
        ereport(LOG,
                (errmsg("%s: fsync file \"%s\" fail", progname, temppath)));
    }
    fclose(statef);

    if (rename(temppath, gaussdb_state_file) != 0)
        ereport(LOG, (errmsg("can't rename \"%s\" to \"%s\": %m", temppath,
                             gaussdb_state_file)));
}
...
static void PMUpdateDBStateHaRebuildReason(void) {
    ...
    PMSetDBStateFile(&state);
    ereport(LOG, (errmsg("update gaussdb state file: build reason(%s), "
...
static int init_stream_comm() {
    ...
    g_instance.comm_cxt.comm_global_mem_cxt = AllocSetContextCreate(
        g_instance.instance_context, "CommunnicatorGlobalMemoryContext",
        ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT);
    ...
    foreach (l, elemlist) {
        ...
        if (strcmp(curhost, "*") != 0 && strcmp(curhost, "localhost") != 0 &&
            strcmp(curhost, "0.0.0.0") != 0) {
    ...
    error =
        gs_set_basic_info(local_ip, g_instance.attr.attr_common.PGXCNodeName,
...
Datum set_working_grand_version_num_manually(PG_FUNCTION_ARGS) {
    ...
    if (u_sess->upg_cxt.InplaceUpgradeSwitch)
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_OPERATION),
                        errmsg("could not set WorkingGrandVersionNum manually "
                               "while not performing upgrade")));
...
#define MAXENVLEN 1024
    ...
    for (i = 0; danger_character_list[i] != NULL; i++) {
        if (strstr((const char*)input_env_value, danger_character_list[i])) {
            ereport(LOG,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                     errmsg("Error: environment variable $%s(%s) contain "
                            "invaild symbol \"%s\".\n",
...
#define MAXENVLEN 1024
    ...
    if (input_env_value == NULL || strlen(input_env_value) >= MAXENVLEN) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("wrong environment variable \"%s\"", input_env_value)));
    }

    for (i = 0; danger_character_list[i] != NULL; i++) {
        if (strstr((const char*)input_env_value, danger_character_list[i])) {
            ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                            errmsg("Error: environment variable \"%s\" contain "
                                   "invaild symbol \"%s\".\n",
...
void CleanSystemCaches(bool is_in_read_command) {
    ...
    usedSize = ((AllocSet)u_sess->cache_mem_cxt)->totalSpace -
               ((AllocSet)u_sess->cache_mem_cxt)->freeSpace;
...
static void SetAuxType() {
    switch (thread_role) {
        ...
        case WALWRITER:
            ...
            break;
        // add by singheart
        case NEU_TRANSACTION_SENDER:
            t_thrd.bootstrap_cxt.MyAuxProcType = NeuTransactionSenderProcess;
            break;
        case NEU_RESPONSE_RECEIVER:
            t_thrd.bootstrap_cxt.MyAuxProcType = NeuResponseReceiverProcess;
            break;
        case NEU_APPLY_LOG_RECEIVER:
            t_thrd.bootstrap_cxt.MyAuxProcType = NeuApplyLogReceiverProcess;
            break;
...
void SetExtraThreadInfo(knl_thread_arg* arg) {
    ...
    switch (role) {
        case THREADPOOL_WORKER: {
            ...
            t_thrd.threadpool_cxt.group =
                t_thrd.threadpool_cxt.worker->GetGroup();
            break;
        }
        case STREAM_WORKER: {
        ...
        case THREADPOOL_STREAM: {
            ...
            t_thrd.threadpool_cxt.group =
                t_thrd.threadpool_cxt.stream->GetGroup();
            StreamProducer* proObj =
                (StreamProducer*)t_thrd.threadpool_cxt.stream->GetProducer();
            SetStreamWorkerInfo(proObj);
            break;
        }
#ifdef ENABLE_MULTIPLE_NODES
        ...
        case THREADPOOL_SCHEDULER: {
            t_thrd.threadpool_cxt.scheduler =
                (ThreadPoolScheduler*)arg->payload;
            break;
        }
        case PAGEREDO: {
...
int GaussDbAuxiliaryThreadMain(knl_thread_arg* arg) {
     ...
     * can occur here.
     */
    srandom((unsigned int)(t_thrd.proc_cxt.MyProcPid ^
                           (unsigned int)t_thrd.proc_cxt.MyStartTime));
    ...
    switch (thread_role) {
        ...
        // add by singheart
        case NEU_TRANSACTION_SENDER:
            NeuTransactionSenderMain();
            break;
        case NEU_RESPONSE_RECEIVER:
            NeuResponseReceiverMain();
            break;
        case NEU_APPLY_LOG_RECEIVER:
            NeuApplyLogReceiverMain();
            break;
...
#endif /* ENABLE_MULTIPLE_NODES */
        default:
            ereport(PANIC, (errmsg("unrecognized process type: %d",
                                   (int)t_thrd.bootstrap_cxt.MyAuxProcType)));
            proc_exit(1);
    }
...
int GaussDbThreadMain(knl_thread_arg* arg) {
    ...
    if (thread_role == TS_COMPACTION || thread_role == TS_COMPACTION_CONSUMER ||
        ...
        u_sess = SessionControl::create_compaction_session_context(
            t_thrd.top_mem_cxt);
    }
#endif /* ENABLE_MULTIPLE_NODES */

    t_thrd.proc_cxt.MyProcPid =
        gs_thread_self(); /* reset t_thrd.proc_cxt.MyProcPid */
    t_thrd.proc_cxt.MyStartTime = time(NULL);
    t_thrd.role = arg->role;
    ...
    if (gs_signal_createtimer() < 0) {
        ereport(FATAL, (errmsg("create timer fail at thread : %lu",
                               t_thrd.proc_cxt.MyProcPid)));
    }
    ...
    if ((thread_role != WORKER && thread_role != THREADPOOL_WORKER &&
         thread_role != STREAM_WORKER) &&
    ...
    t_thrd.wlm_cxt.query_resource_track_mcxt = AllocSetContextCreate(
        t_thrd.top_mem_cxt, "QueryResourceTrackContext",
        ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
    ...
    switch (thread_role) {
        ...
        case THREADPOOL_STREAM: {
            ...
            if (thread_role == STREAM_WORKER) {
                t_thrd.proc_cxt.MyPMChildSlot =
                    u_sess->stream_cxt.producer_obj->getChildSlot();
            } else {
                t_thrd.proc_cxt.MyPMChildSlot =
                    t_thrd.threadpool_cxt.stream->GetProducer()->getChildSlot();
            }
        ...
        case THREADPOOL_WORKER: {
             ...
             */
            if (u_sess->stream_cxt.stop_mythread) {
                t_thrd.sig_cxt.gs_sigale_check_type =
                    SIGNAL_CHECK_EXECUTOR_STOP;
                SendProcSignal(u_sess->stream_cxt.stop_pid,
                               PROCSIG_EXECUTOR_FLAG, InvalidBackendId);
        ...
        default:
            ereport(PANIC,
                    (errmsg("unsupport thread role type %d", arg->role)));
            break;
    }
...
static ThreadMetaData GaussdbThreadGate[] = {
    ...
    {GaussDbThreadMain<THREADPOOL_WORKER>, THREADPOOL_WORKER, "TPLworker",
     "thread pool worker"},
    {GaussDbThreadMain<THREADPOOL_LISTENER>, THREADPOOL_LISTENER, "TPLlistener",
     "thread pool listner"},
    {GaussDbThreadMain<THREADPOOL_SCHEDULER>, THREADPOOL_SCHEDULER,
     "TPLscheduler", "thread pool scheduler"},
    {GaussDbThreadMain<THREADPOOL_STREAM>, THREADPOOL_STREAM, "TPLstream",
     "thread pool stream"},
    {GaussDbThreadMain<STREAM_WORKER>, STREAM_WORKER, "streamworker",
     "stream worker"},
    {GaussDbThreadMain<AUTOVACUUM_LAUNCHER>, AUTOVACUUM_LAUNCHER, "AVClauncher",
     "autovacuum launcher"},
    {GaussDbThreadMain<AUTOVACUUM_WORKER>, AUTOVACUUM_WORKER, "AVCworker",
     "autovacuum worker"},
    {GaussDbThreadMain<JOB_SCHEDULER>, JOB_SCHEDULER, "Jobscheduler",
     "job scheduler"},
    {GaussDbThreadMain<JOB_WORKER>, JOB_WORKER, "Jobworker",
     "job execute worker"},
    {GaussDbThreadMain<WLM_WORKER>, WLM_WORKER, "WLMworker",
     "wlm statistics collector"},
    {GaussDbThreadMain<WLM_MONITOR>, WLM_MONITOR, "WLMmonitor",
     "wlm monitor launcher"},
    {GaussDbThreadMain<WLM_ARBITER>, WLM_ARBITER, "WLMarbiter",
     "wlm arbiter launcher"},
    {GaussDbThreadMain<WLM_CPMONITOR>, WLM_CPMONITOR, "CPmonitor",
     "CPmonitor launcher"},
    {GaussDbThreadMain<AUDITOR>, AUDITOR, "auditor", "system auditor"},
    {GaussDbThreadMain<PGSTAT>, PGSTAT, "statscollector",
    ...
    {GaussDbThreadMain<RPC_SERVICE>, RPC_SERVICE, "rpcservice",
     "remote service"},
    ...
    {GaussDbThreadMain<TWOPASECLEANER>, TWOPASECLEANER, "2pccleaner",
     "twophase cleaner"},
    {GaussDbThreadMain<STARTUP>, STARTUP, "startup", "startup"},
    {GaussDbThreadMain<FAULTMONITOR>, FAULTMONITOR, "faultmonitor",
     "fault monitor"},
    {GaussDbThreadMain<BGWRITER>, BGWRITER, "bgwriter", "background writer"},
    {GaussDbThreadMain<PERCENTILE_WORKER>, PERCENTILE_WORKER, "percentworker",
     "statistics collector"},
    {GaussDbThreadMain<SNAPSHOT_WORKER>, SNAPSHOT_WORKER, "snapshotworker",
     "snapshot"},
    {GaussDbThreadMain<ASH_WORKER>, ASH_WORKER, "ashworker", "ash worker"},
    {GaussDbThreadMain<TRACK_STMT_WORKER>, TRACK_STMT_WORKER, "TrackStmtWorker",
     "track stmt worker"},
    {GaussDbThreadMain<TRACK_STMT_CLEANER>, TRACK_STMT_CLEANER,
     "TrackStmtClean", "track stmt clean worker"},
    {GaussDbThreadMain<CHECKPOINT_THREAD>, CHECKPOINT_THREAD, "checkpointer",
     "checkpointer"},
    {GaussDbThreadMain<WALWRITER>, WALWRITER, "WALwriter", "WAL writer"},

    // add by singheart
    {GaussDbThreadMain<NEU_TRANSACTION_SENDER>, NEU_TRANSACTION_SENDER, "NeuTxnSender", "NEU Transaction Sender"},
    {GaussDbThreadMain<NEU_RESPONSE_RECEIVER>, NEU_RESPONSE_RECEIVER, "NeuRspReceiver", "NEU Response Receiver"},
    {GaussDbThreadMain<NEU_APPLY_LOG_RECEIVER>, NEU_APPLY_LOG_RECEIVER, "NeuLogReceiver", "NEU Apply Log Receiver"},

    {GaussDbThreadMain<WALWRITERAUXILIARY>, WALWRITERAUXILIARY, "WALwriteraux",
     "WAL writer auxiliary"},
    {GaussDbThreadMain<WALRECEIVER>, WALRECEIVER, "WALreceiver",
     "WAL receiver"},
    {GaussDbThreadMain<WALRECWRITE>, WALRECWRITE, "WALrecwriter",
     "WAL receive writer"},
    {GaussDbThreadMain<DATARECIVER>, DATARECIVER, "datareceiver",
     "data receiver"},
    {GaussDbThreadMain<DATARECWRITER>, DATARECWRITER, "datarecwriter",
     "data receive writer"},
    {GaussDbThreadMain<CBMWRITER>, CBMWRITER, "CBMwriter", "CBM writer"},
    {GaussDbThreadMain<PAGEWRITER_THREAD>, PAGEWRITER_THREAD, "pagewriter",
     "page writer"},
    {GaussDbThreadMain<HEARTBEAT>, HEARTBEAT, "heartbeat", "heart beat"},
    {GaussDbThreadMain<COMM_SENDERFLOWER>, COMM_SENDERFLOWER, "COMMsendflow",
     "communicator sender flower"},
    {GaussDbThreadMain<COMM_RECEIVERFLOWER>, COMM_RECEIVERFLOWER,
     "COMMrecvflow", "communicator receiver flower"},
    {GaussDbThreadMain<COMM_RECEIVER>, COMM_RECEIVER, "COMMrecloop",
     "communicator receiver loop"},
    {GaussDbThreadMain<COMM_AUXILIARY>, COMM_AUXILIARY, "COMMaux",
     "communicator auxiliary"},
    {GaussDbThreadMain<COMM_POOLER_CLEAN>, COMM_POOLER_CLEAN, "COMMpoolcleaner",
     "communicator pooler auto cleaner"},
    {GaussDbThreadMain<CSNMIN_SYNC>, CSNMIN_SYNC, "csnminsync", "csnmin sync"},
    {GaussDbThreadMain<BARRIER_CREATOR>, BARRIER_CREATOR, "barriercreator",
     "barrier creator"},
...
/* Keep the block in the end if it may be absent !!! */
    ...
    {GaussDbThreadMain<TS_COMPACTION_CONSUMER>, TS_COMPACTION_CONSUMER,
     "TScompconsumer", "timeseries consumer compaction"},
    {GaussDbThreadMain<TS_COMPACTION_AUXILIAY>, TS_COMPACTION_AUXILIAY,
     "TScompaux", "compaction auxiliary"},
    {GaussDbThreadMain<STREAMING_BACKEND>, STREAMING_BACKEND, "streambackend",
     "streaming backend"},
    {GaussDbThreadMain<STREAMING_ROUTER_BACKEND>, STREAMING_ROUTER_BACKEND,
     "streamrouter", "streaming router backend"},
    {GaussDbThreadMain<STREAMING_WORKER_BACKEND>, STREAMING_WORKER_BACKEND,
     "streamworker", "streaming worker backend"},
    {GaussDbThreadMain<STREAMING_COLLECTOR_BACKEND>,
     STREAMING_COLLECTOR_BACKEND, "streamcollector",
     "streaming collector backend"},
    {GaussDbThreadMain<STREAMING_QUEUE_BACKEND>, STREAMING_QUEUE_BACKEND,
     "streamqueue", "streaming queue backend"},
    {GaussDbThreadMain<STREAMING_REAPER_BACKEND>, STREAMING_REAPER_BACKEND,
     "streamreaper", "streaming reaper backend"}
#endif /* ENABLE_MULTIPLE_NODES */
};
...
ThreadId initialize_thread(ThreadArg* thr_argv) {
    ...
    gs_thread_t thread;
    int error_code =
        gs_thread_create(&thread, InternalThreadFunc, 1, (void*)thr_argv);
    if (error_code != 0) {
        ereport(LOG,
...
ThreadId initialize_util_thread(knl_thread_role role, void* payload) {
    ...
    if (!save_backend_variables(
            (BackendParameters*)(thr_argv->m_thd_arg.save_para), &port)) {
        gs_thread_release_args_slot(thr_argv);
    ...
    if (role == SYSLOGGER) {
        if (t_thrd.logger.syslogFile != NULL)
            thr_argv->m_thd_arg.extra_payload.log_thread.syslog_handle =
                fileno(t_thrd.logger.syslogFile);
        else
...
ThreadId initialize_worker_thread(knl_thread_role role, Port* port,
                                  void* payload) {
    ThreadArg* thr_argv = gs_thread_get_args_slot();
    if (thr_argv == NULL) {
        return InvalidTid;
...
Datum disable_conn(PG_FUNCTION_ARGS) {
    ...
    if (arg0 == NULL) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_ATTRIBUTE),
                 errmsg("Invalid null pointer attribute for disable_conn()")));
    }
    char* host = NULL;

    if (getObsReplicationSlot() && IsServerModeStandby() &&
        !XLogArchivingActive()) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_OPERATION),
    ...
    if (!superuser()) {
        ereport(ERROR, (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
                        errmsg("must be superuser/sysadmin account to perform "
                               "disable_conn()")));
    }
    ...
    if (0 == strcmp(disconn_mode, POLLING_CONNECTION_STR)) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_OPERATION),
                 errmsg("Connection mode should be polling_connection or "
                        "specify_connection or prohibit_connection")));
    }
    ...
    if (disconn_node.conn_mode == PROHIBIT_CONNECTION) {
        ...
        if (!redoDone) {
            ...
            ereport(ERROR, (errcode_for_file_access(),
                            errmsg("could not add lock when DN is not redo all "
                                   "xlog, redo done flag is false")));
        }
        ...
        if (replay1 != replay2) {
            ereport(
                ERROR,
                (errcode_for_file_access(),
                 errmsg("could not add lock when DN is not redo all xlog.")));
        }
    } else {
    ...
    if (disconn_node.conn_mode != SPECIFY_CONNECTION) {
        ...
        if (!isVaildIp(host)) {
            ereport(ERROR, (errcode(ERRCODE_INVALID_OPERATION),
                            errmsg("host is invalid")));
        }
        errno_t rc = memcpy_s(disconn_node.disable_conn_node_host, NAMEDATALEN,
                              host, strlen(host) + 1);
        securec_check(rc, "\0", "\0");
    }
    if (disconn_node.conn_mode != SPECIFY_CONNECTION) {
    ...
    int fd;
    fd = BasicOpenFile(disable_conn_file, O_CREAT | O_WRONLY | PG_BINARY,
                       S_IRUSR | S_IWUSR);
    if (fd < 0) {
        ereport(ERROR, (errcode_for_file_access(),
    ...
    if (write(fd, &disconn_node, sizeof(disconn_node)) !=
        sizeof(disconn_node)) {
        pgstat_report_waitevent(WAIT_EVENT_END);
        close(fd);
        ereport(ERROR, (errcode_for_file_access(),
    ...
    if (pg_fsync(fd) != 0) {
        ...
        ereport(ERROR,
                (errcode_for_file_access(),
                 errmsg("could not fsync file \"%s\": %m", disable_conn_file)));
    }
    pgstat_report_waitevent(WAIT_EVENT_END);
    close(fd);

    SpinLockAcquire(
        &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
    g_instance.comm_cxt.localinfo_cxt.disable_conn_node.disable_conn_node_data =
        disconn_node;
    SpinLockRelease(
        &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
    ereport(LOG, (errcode(ERRCODE_LOG),
                  errmsg("disable_conn set mode to %s", disconn_mode)));
    PG_RETURN_VOID();
}
...
Datum read_disable_conn_file(PG_FUNCTION_ARGS) {
    ...
    securec_check(rc, "\0", "\0");
    if (t_thrd.postmaster_cxt.ReplConnChanged[1] == false ||
        u_sess->attr.attr_storage.ReplConnInfoArr[1] == NULL) {
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg("Can't get local connection address.")));
    }
    rc = memcpy_s(local_info, NAMEDATALEN - 1,
                  u_sess->attr.attr_storage.ReplConnInfoArr[1],
                  MAX_LOCAL_ADDRESS_LENGTH);
    securec_check(rc, "\0", "\0");
    key_position = strtok_s(local_info, " ", &next_key);
    if (key_position == NULL || sscanf_s(key_position, "localhost=%s",
                                         local_host, NAMEDATALEN - 1) != 1) {
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg("get local host failed!")));
    }

    key_position = strtok_s(NULL, " ", &next_key);
    if (key_position == NULL || sscanf_s(key_position, "localport=%s",
                                         local_port, NAMEDATALEN - 1) != 1) {
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                        errmsg("get local port failed!")));
    }
    ...
    SpinLockAcquire(
        &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
    switch (g_instance.comm_cxt.localinfo_cxt.disable_conn_node
                .disable_conn_node_data.conn_mode) {
    ...
    rc = snprintf_s(disconn_node_port_str, sizeof(disconn_node_port_str),
                    sizeof(disconn_node_port_str) - 1, "%d",
                    g_instance.comm_cxt.localinfo_cxt.disable_conn_node
                        .disable_conn_node_data.disable_conn_node_port);
    securec_check_ss_c(rc, "\0", "\0");
    values[1] =
        CStringGetTextDatum(g_instance.comm_cxt.localinfo_cxt.disable_conn_node
                                .disable_conn_node_data.disable_conn_node_host);
...
void set_disable_conn_mode() {
    ...
    size_t cnt = 0;
    knl_g_disconn_node_context_data* disconn_node =
        (knl_g_disconn_node_context_data*)palloc(
            sizeof(knl_g_disconn_node_context_data));
    ...
    if (fd >= 0) {
        ...
        if (cnt != sizeof(knl_g_disconn_node_context_data)) {
            ereport(ERROR,
                    (0, errmsg("cannot read disable connection file: \"%s\" \n",
                               disable_conn_file)));
        }
        ...
        SpinLockAcquire(
            &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
        g_instance.comm_cxt.localinfo_cxt.disable_conn_node
            .disable_conn_node_data.conn_mode = disconn_node->conn_mode;
        g_instance.comm_cxt.localinfo_cxt.disable_conn_node
            .disable_conn_node_data.disable_conn_node_port =
            disconn_node->disable_conn_node_port;

        errno_t rc =
            memcpy_s((void*)g_instance.comm_cxt.localinfo_cxt.disable_conn_node
                         .disable_conn_node_data.disable_conn_node_host,
                     NAMEDATALEN, (void*)disconn_node->disable_conn_node_host,
                     NAMEDATALEN);
        securec_check(rc, "\0", "\0");
        SpinLockRelease(
            &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
        ...
        SpinLockAcquire(
            &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
        g_instance.comm_cxt.localinfo_cxt.disable_conn_node
            .disable_conn_node_data.conn_mode = POLLING_CONNECTION;
        g_instance.comm_cxt.localinfo_cxt.disable_conn_node
            .disable_conn_node_data.disable_conn_node_host[0] = 0;
        g_instance.comm_cxt.localinfo_cxt.disable_conn_node
            .disable_conn_node_data.disable_conn_node_port = 0;
        SpinLockRelease(
            &g_instance.comm_cxt.localinfo_cxt.disable_conn_node.info_lck);
    }
...
static bool NeedHeartbeat() {
    ...
    for (int i = 1; i < MAX_REPLNODE_NUM; i++) {
        ...
        if (replconn != NULL && replconn->localheartbeatport != 0 &&
            replconn->remoteheartbeatport != 0) {
            return true;
        }
    }
    return false;
...
void GenerateCancelKey(bool isThreadPoolSession) {
    if (isThreadPoolSession) {
        ...
        t_thrd.proc_cxt.MyCancelKey =
            (unsigned long)t_thrd.proc_cxt.MyCancelKey | 0x1;
    }
}
