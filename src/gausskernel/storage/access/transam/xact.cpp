...
#include "pg_trace.h"
#include "../../mot/core/src/system/global.h"
...
/* just need cancel stmt once when DefaultXactReadOnly=true */
...
static void CallSubXactCallbacks(SubXactEvent event, SubTransactionId mySubid,
...
static void SendOneSavepointToRemoteCoordinators(
    const char* cmd, const char* name, SavepointStmtType stmtType,
...
void CleanUpDnHashTable(void) {
    if ((StreamThreadAmI() && StreamTopConsumerAmI()) || !StreamThreadAmI()) {
        if (IsUnderPostmaster && (GetUserId() != BOOTSTRAP_SUPERUSERID)) {
            ...
            hash_search(g_instance.comm_cxt.usedDnSpace,
                        &u_sess->debug_query_id, HASH_REMOVE, NULL);
...
void RemoveFromDnHashTable(void) {
    if (IS_PGXC_DATANODE && (u_sess->debug_query_id != 0) && (
            u_sess->attr.attr_resource.sqlUseSpaceLimit > 0)) {
        CleanUpDnHashTable();
    }
}
...
TransactionId GetCurrentTransactionId(void) {
    ...
    if (!TransactionIdIsValid(s->transactionId))
        ereport(
        ERROR,
        (errcode(ERRCODE_IN_FAILED_SQL_TRANSACTION), errmsg("Xid is invalid.")
        ));
...
GTM_Timeline GetCurrentTransactionTimeline(void) {
    Assert(
        GlobalTransactionTimelineIsValid(CurrentTransactionState->txnKey.
            txnTimeline));
...
static void AssignTransactionId(TransactionState s) {
    ...
    if (isSubXact && !TransactionIdIsValid(s->parent->transactionId)) {
        ...
        parents = (TransactionState*)palloc(
            sizeof(TransactionState) * s->nestingLevel);
     ...
     * xl_standby_locks.
     */
    if (isSubXact && XLogLogicalInfoActive() && !TopTransactionStateData.
        didLogXid)
    ...
    if (isSubXact && XLogStandbyInfoActive()) {
        t_thrd.xact_cxt.unreportedXids[t_thrd.xact_cxt.nUnreportedXids] = s->
            transactionId;
         ...
         */
        if (t_thrd.xact_cxt.nUnreportedXids >= PGPROC_MAX_CACHED_SUBXIDS ||
            log_unknown_top) {
            ...
            XLogRegisterData((char*)t_thrd.xact_cxt.unreportedXids,
                             t_thrd.xact_cxt.nUnreportedXids * sizeof(
                                 TransactionId));
...
TimestampTz GetCurrentStatementStartTimestamp(void) {
     ...
     */
#ifdef PGXC
    return t_thrd.xact_cxt.stmtStartTimestamp + t_thrd.xact_cxt.
           GTMdeltaTimestamp;
#else
...
TimestampTz GetCurrentTransactionStopTimestamp(void) {
    ...
    if (t_thrd.xact_cxt.xactStopTimestamp != 0)
        return t_thrd.xact_cxt.xactStopTimestamp + t_thrd.xact_cxt.
               GTMdeltaTimestamp;
...
void SetCurrentGTMDeltaTimestamp(void) {
    t_thrd.xact_cxt.GTMdeltaTimestamp =
        t_thrd.xact_cxt.GTMxactStartTimestamp - t_thrd.xact_cxt.
        stmtStartTimestamp;
}
...
void SetStmtSysGTMDeltaTimestamp(void) {
    stmtSysGTMdeltaTimestamp = t_thrd.xact_cxt.GTMxactStartTimestamp - t_thrd.
                               time_cxt.stmt_system_timestamp;
}
...
static void AtStart_Memory(void) {
    ...
    if (t_thrd.xact_cxt.TransactionAbortContext == NULL)
        t_thrd.xact_cxt.TransactionAbortContext = AllocSetContextCreate(
            t_thrd.top_mem_cxt, "TransactionAbortContext",
    ...
    u_sess->top_transaction_mem_cxt = AllocSetContextCreate(
        u_sess->top_mem_cxt, "TopTransactionContext",
...
static void AtStart_ResourceOwner(void) {
    ...
    s->curTransactionOwner = ResourceOwnerCreate(
        NULL, "TopTransaction", MEMORY_CONTEXT_STORAGE);
...
static void AtSubStart_Memory(void) {
     ...
     */
    t_thrd.mem_cxt.cur_transaction_mem_cxt = AllocSetContextCreate(
...
CommitSeqNo getLocalNextCSN() {
    return pg_atomic_fetch_add_u64(
        &t_thrd.xact_cxt.ShmemVariableCache->nextCommitSeqNo, 1);
}
...
loop:
    currentNextCommitSeqNo = pg_atomic_read_u64(
...
static TransactionId RecordTransactionCommit(void) {
    ...
    if (XLogStandbyInfoActive())
        nmsgs = xactGetCommittedInvalidationMessages(
    ...
    if (!markXidCommitted) {
        ...
        SetXact2CommitInProgress(xid, 0);
            ...
            if (TransactionIdIsNormal(xid) &&
                (!(useLocalXid || !IsPostmasterEnvironment || GTM_FREE_MODE ||
                   GetForceXidFromGTM())) &&
                (GetCommitCsn() == 0)) {
                ...
                ereport(
                    LOG, (errmsg(
                            "Set a new csn from gtm for auto commit transactions."
                        )
                    ));
                if (GTM_MODE)
         ...
         */
        if (nrels > 0 || nmsgs > 0 || RelcacheInitFileInval || t_thrd.xact_cxt.
            forceSyncCommit ||
            ...
            xlrec.xact_time = t_thrd.xact_cxt.xactStopTimestamp + t_thrd.
                              xact_cxt.GTMdeltaTimestamp;
#else
...
#ifndef ENABLE_MULTIPLE_NODES
            XLogRegisterData((char*)&u_sess->utils_cxt.RecentXmin,
            ...
            if (nrels > 0) {
                ...
                if (!XLogRecPtrIsInvalid(globalDelayDDLLSN) && XLByteLT(
                        globalDelayDDLLSN, commitRecLSN))
     ...
     * anyway if we crash.)
     */
    if ((wrote_xlog && synchronous_commit > SYNCHRONOUS_COMMIT_OFF) || t_thrd.
        xact_cxt.forceSyncCommit || nrels > 0) {
        /*
     ...
     */
    if (wrote_xlog && u_sess->attr.attr_storage.guc_synchronous_commit >
...
static void AtSubCommit_childXids(void) {
    ...
    if (s->nChildXids > 0) {
        errorno = memcpy_s(&s->parent->childXids[s->parent->nChildXids + 1],
                           (unsigned)s->parent->maxChildXids * sizeof(
                               TransactionId), s->childXids,
...
static TransactionId RecordTransactionAbort(bool isSubXact) {
    ...
    if (bCanAbort && TransactionIdDidCommit(xid))
        ereport(PANIC, (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                errmsg("cannot abort transaction %lu, it was already committed",
                    xid)));
...
static void AtAbort_ResourceOwner(void) {
     ...
     * NULL, which is OK)
     */
    t_thrd.utils_cxt.CurrentResourceOwner = t_thrd.utils_cxt.
...
static void AtCleanup_Memory(void) {
    Assert(
        (!StreamThreadAmI() && CurrentTransactionState->parent == NULL) ||
        StreamThreadAmI());
...
static void StartTransaction(bool begin_on_gtm) {
    ...
    if (s->state != TRANS_DEFAULT) {
        ereport(WARNING,
                (errmsg("StartTransaction while in %s state", TransStateAsString
                    (s->state))));
    }
    ...
    if (RecoveryInProgress()) {
        ...
        u_sess->attr.attr_common.XactReadOnly = u_sess->attr.attr_storage.
            DefaultXactReadOnly;
#ifdef PGXC
    ...
    u_sess->attr.attr_storage.XactDeferrable = u_sess->attr.attr_storage.
...
#endif
    u_sess->utils_cxt.XactIsoLevel = u_sess->attr.attr_common.
                                             DefaultXactIsoLevel;
    ...
    bool normal_working = begin_on_gtm && IsNormalProcessingMode() &&
                          /* not in CommitTransaction and InitPostgres */
                          !(IsAutoVacuumWorkerProcess() && (
                                t_thrd.pgxact->vacuumFlags & PROC_IN_VACUUM));
    bool update_xact_time = !GTM_FREE_MODE && !u_sess->attr.attr_common.
                            xc_maintenance_mode && normal_working &&
     ...
     */
    update_xact_time =
        (GTM_FREE_MODE && IS_PGXC_COORDINATOR && !IsConnFromCoord()) ||
        (!t_thrd.xact_cxt.timestamp_from_cn && IS_PGXC_DATANODE &&
         normal_working);
    if (update_xact_time) {
        t_thrd.xact_cxt.GTMxactStartTimestamp = t_thrd.xact_cxt.
            xactStartTimestamp;
    ...
    if (module_logging_is_on(MOD_TRANS_XACT)) {
        ereport(LOG, (errmodule(MOD_TRANS_XACT),
                    errmsg(
                        "start transaction succ. In Node %s, trans state: %s -> %s."
                        ,
...
static void CommitTransaction(bool STP_commit) {
    ...
    bool use_old_version_gid =
        GTM_MODE || (t_thrd.proc->workingVersionNum <= GTM_OLD_VERSION_NUM);
#ifdef ENABLE_MULTIPLE_NODES
    ...
    if (needNewLocalCacheFile) {
        ereport(WARNING,
                (errcode(ERRCODE_WARNING), errmsg(
                        "Wrong flag of relcache init flag at commit transaction."
                    )
                ));

        needNewLocalCacheFile = false;
        pg_atomic_exchange_u32(
            &t_thrd.xact_cxt.ShmemVariableCache->CriticalCacheBuildLock, 0);
    }
    ...
    if (s->state != TRANS_INPROGRESS)
        ereport(WARNING, (errcode(ERRCODE_WARNING),
                errmsg("CommitTransaction while in %s state", TransStateAsString
                    (s->state))));
     ...
     */
    if (STP_commit) {
        ResourceOwnerDecrementNPlanRefs(t_thrd.utils_cxt.CurrentResourceOwner,
                                        true);
    }
#ifdef PGXC
    /*
    ...
    if (IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
        ...
        if (module_logging_is_on(MOD_TRANS_XACT)) {
            ereport(LOG, (errmodule(MOD_TRANS_XACT),
                        errmsg("reserved xid for commit check is %lu", t_thrd.
                            xact_cxt.XactXidStoreForCheck)));
        }
        ...
        if (IsTwoPhaseCommitRequired(t_thrd.xact_cxt.XactWriteLocalNode)) {
            errno_t errorno = EOK;
            u_sess->xact_cxt.prepareGID = (char*)MemoryContextAlloc(
                u_sess->top_transaction_mem_cxt, MAX_GID_LENGTH);
            if (use_old_version_gid) {
                errorno = snprintf_s(u_sess->xact_cxt.prepareGID,
                                     MAX_GID_LENGTH, MAX_GID_LENGTH - 1,
                                     "T%lu_%s",
                                     GetTopTransactionId(),
                                     g_instance.attr.attr_common.PGXCNodeName);
            } else {
    ...
    if (module_logging_is_on(MOD_TRANS_XACT)) {
        ereport(LOG, (errmodule(MOD_TRANS_XACT),
                    errmsg(
                        "Local Node %s: local commit has written clog and xlog, trans state : %s -> %s"
                        ,
                        g_instance.attr.attr_common.PGXCNodeName,
    ...
    ResourceOwnerRelease(t_thrd.utils_cxt.TopTransactionResourceOwner,
    ...
    if (module_logging_is_on(MOD_TRANS_XACT)) {
        ereport(LOG, (errmodule(MOD_TRANS_XACT),
                    errmsg(
                        "Local Node %s: local commit process completed, trans state : %s -> %s"
                        ,
                        g_instance.attr.attr_common.PGXCNodeName,
                        TransStateAsString(oldstate),
                        TransStateAsString(s->state))));
...
static void PrepareTransaction(bool STP_commit) {
    ...
    if (s->state != TRANS_INPROGRESS)
        ereport(WARNING, (errcode(ERRCODE_WARNING),
                errmsg("PrepareTransaction while in %s state",
                    TransStateAsString(s->state))));
    Assert((!StreamThreadAmI() && s->parent == NULL) || StreamThreadAmI());
    ...
    if (GTM_MODE && (IS_PGXC_DATANODE || IsConnFromCoord())) {
        ...
        if (topTransactionId != GetTransactionIdFromGidStr(
                u_sess->xact_cxt.prepareGID)) {
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                        errmsg(
                            ...
                            ,
                            g_instance.attr.attr_common.PGXCNodeName, u_sess->
                            xact_cxt.prepareGID, topTransactionId)));
        }
    }
    ...
    if (IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
        ...
        nodestring = PrePrepare_Remote(u_sess->xact_cxt.savePrepareGID,
                                       isImplicit, true);
     ...
     */
    gxact = MarkAsPreparing(handle, xid, u_sess->xact_cxt.prepareGID,
                            prepared_at, GetUserId(),
                            u_sess->proc_cxt.MyDatabaseId,
                            t_thrd.proc->sessionid);
        ...
        if (!GTM_FREE_MODE && !AtEOXact_GlobalTxn(true, true)) {
            ereport(
                ...
                (errcode(ERRCODE_CONNECTION_EXCEPTION),
                    errmsg(
                        "Failed to receive GTM commit transaction response after CN local PREPARE TRANSACTION '%s'."
                        ,
                        u_sess->xact_cxt.savePrepareGID)));
        }
    }
     ...
     */
    if (IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
        PostPrepare_Remote(u_sess->xact_cxt.savePrepareGID, nodestring,
                           isImplicit);
        if (!isImplicit) {
            s->txnKey.txnHandle = InvalidTransactionHandle;
        }
...
static void AbortTransaction(bool PerfectRollback, bool STP_rollback) {
    ...
    if (IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
        ...
        if (module_logging_is_on(MOD_TRANS_XACT)) {
            ereport(LOG, (errmodule(MOD_TRANS_XACT),
                        errmsg("reserved xid for abort check is %lu", t_thrd.
                            xact_cxt.XactXidStoreForCheck)));
        }
    }
     ...
     */
    if (!GTM_FREE_MODE) {
        if (t_thrd.xact_cxt.XactLocalNodeCanAbort && !AtEOXact_GlobalTxn(
                false, false) &&
            !t_thrd.proc_cxt.proc_exit_inprogress) {
            ereport(WARNING,
                    (errmsg(
                        "Failed to receive GTM rollback transaction response  for aborting prepared %s."
                        ,
                        u_sess->xact_cxt.savePrepareGID)));
        }
    }
...
#endif
    if (t_thrd.xact_cxt.XactLocalNodePrepared && t_thrd.xact_cxt.
        XactLocalNodeCanAbort) {
#ifdef ENABLE_DISTRIBUTE_TEST
    ...
    if (IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
        ...
        if (t_thrd.xact_cxt.XactLocalNodeCanAbort) {
            ...
            ereport(
                DEBUG1,
                (errmsg(
                    "AbortTransaction in commit state, not to call sequence call back"
                )));
        }
    }
#endif
    ...
    if (s->state != TRANS_INPROGRESS && s->state != TRANS_PREPARE)
        ereport(WARNING,
            (errcode(ERRCODE_WARNING), errmsg(
                "AbortTransaction while in %s state", TransStateAsString(s->
                    state))));
    ...
    switch (s->blockState) {
        ...
        case TBLOCK_DEFAULT:
            ...
            if (module_logging_is_on(MOD_TRANS_XACT)) {
                ereport(LOG, (errmodule(MOD_TRANS_XACT),
                            errmsg(
                                "StartTransactionCommand: In Node %s, TransBlock state : %s -> %s"
                                ,
                                g_instance.attr.attr_common.PGXCNodeName,
...
void CommitTransactionCommand(bool STP_commit) {
    ...
    switch (s->blockState) {
        ...
        case TBLOCK_SUBINPROGRESS:
            ...
            if (u_sess->SPI_cxt.portal_stp_exception_counter > 0) {
                ...
                Assert(
                    subTransactionCounter == u_sess->SPI_cxt.
                    portal_stp_exception_counter);
                t_thrd.utils_cxt.CurrentResourceOwner = t_thrd.utils_cxt.
                    STPSavedResourceOwner;
                CommitTransaction(STP_commit);
                s->blockState = TBLOCK_DEFAULT;
            }
            break;
        ...
        case TBLOCK_SUBBEGIN:
            ...
            if (CurrentTransactionState->nestingLevel == 2 && STP_commit) {
                t_thrd.utils_cxt.STPSavedResourceOwner = t_thrd.utils_cxt.
        ...
        case TBLOCK_SUBRELEASE:
            ...
            Assert(
                s->blockState == TBLOCK_INPROGRESS || s->blockState ==
    ...
    if (module_logging_is_on(MOD_TRANS_XACT)) {
        ereport(LOG,
                (errmodule(MOD_TRANS_XACT), errmsg(
                    "CommitTransactionCommand: TransBlock state %s -> %s",
                    BlockStateAsString(oldstate), BlockStateAsString(s->
                        blockState))));
    }
}
...
void AbortCurrentTransaction(bool STP_rollback) {
    ...
    switch (s->blockState) {
        ...
        default:
            ereport(FATAL, (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                        errmsg("AbortCurrentTransaction: unexpected state %s",
                            BlockStateAsString(s->blockState))));
            break;
    }
}
...
void PreventTransactionChain(bool isTopLevel, const char* stmtType) {
    ...
    if (CurrentTransactionState->blockState != TBLOCK_DEFAULT &&
        ...
        ereport(
        FATAL,
        (errcode(ERRCODE_INVALID_TRANSACTION_STATE), errmsg(
...
void UnregisterXactCallback(XactCallback callback, const void* arg) {
    ...
    prev = NULL;
    for (item = u_sess->xact_cxt.Xact_callbacks; item;
...
void UnregisterSubXactCallback(SubXactCallback callback, const void* arg) {
    ...
    prev = NULL;
    for (item = u_sess->xact_cxt.SubXact_callbacks; item;
         prev = item, item = item->next) {
        if (item->callback == callback && item->arg == arg) {
            if (prev != NULL) {
                prev->next = item->next;
...
void RegisterGTMCallback(GTMCallback callback, void* arg) {
    ...
    item = (GTMCallbackItem*)MemoryContextAlloc(u_sess->top_transaction_mem_cxt,
                                                sizeof(GTMCallbackItem));
    item->callback = callback;
    item->arg = arg;
...
void UnregisterGTMCallback(GTMCallback callback, const void* arg) {
    ...
    prev = NULL;
    for (item = t_thrd.xact_cxt.GTM_callbacks; item;
         prev = item, item = item->next) {
        if (item->callback == callback && item->arg == arg) {
            if (prev != NULL) {
                prev->next = item->next;
...
static void CallSequenceCallbacks(GTMEvent event) {
    ...
    PG_TRY();
        {
            for (item = t_thrd.xact_cxt.Seq_callbacks; item;
                 item = item->next) {
                (*item->callback)(event, item->arg);
            }
        }
    PG_CATCH();
        {
            ...
            if (event == GTM_EVENT_ABORT) {
                ...
                ereport(WARNING,
                        (errmsg(
                            "Fail to call sequence call backs when aborting transaction."
                        )));
            } else {
                PG_RE_THROW();
            }
...
static void CleanSequenceCallbacks(void) {
    ...
    for (item = t_thrd.xact_cxt.Seq_callbacks; item; item = next) {
        ...
        if (item->callback == drop_sequence_cb) {
            drop_sequence_callback_arg* cbargs = (drop_sequence_callback_arg*)(
                item->arg);
            if (cbargs != NULL) {
                pfree_ext(cbargs);
            }
        } else if (item->callback == rename_sequence_cb) {
            rename_sequence_callback_arg* rcbargs = (
                rename_sequence_callback_arg*)(item->arg);
            if (rcbargs != NULL) {
...
void BeginTransactionBlock(void) {
    ...
    switch (s->blockState) {
        ...
        case TBLOCK_SUBABORT:
            if ((IS_PGXC_COORDINATOR && !IsConnFromCoord()) ||
                (IS_PGXC_DATANODE == true && IS_SINGLE_NODE == true &&
                 useLocalXid == true)) {
...
bool PrepareTransactionBlock(const char* gid) {
    ...
    if (result) {
        ...
        if (s->blockState == TBLOCK_END) {
            ...
            u_sess->xact_cxt.prepareGID = MemoryContextStrdup(
                u_sess->top_transaction_mem_cxt, gid);
            ...
            if (module_logging_is_on(MOD_TRANS_XACT)) {
                ereport(LOG, (errmodule(MOD_TRANS_XACT),
                            errmsg(
                                "Node %s: state in prepareTransactionBlock is %s"
                                ,
                                g_instance.attr.attr_common.PGXCNodeName,
                                BlockStateAsString(s->blockState))));
            }
...
bool EndTransactionBlock(void) {
    ...
    switch (s->blockState) {
        ...
        case TBLOCK_INPROGRESS:
            ...
            if (module_logging_is_on(MOD_TRANS_XACT)) {
                ereport(LOG, (errmodule(MOD_TRANS_XACT),
                            errmsg("EndTransactionBlock: state %s",
                                BlockStateAsString(s->blockState))));
            }
        ...
        case TBLOCK_STARTED:
            ereport(WARNING,
                    (errcode(ERRCODE_NO_ACTIVE_SQL_TRANSACTION), errmsg(
...
void DefineSavepoint(const char* name) {
    ...
    switch (s->blockState) {
         ...
         */
            if (name != NULL) {
                s->name = MemoryContextStrdup(u_sess->top_transaction_mem_cxt,
                                              name);
            }
            break;
...
void ReleaseSavepoint(List* options) {
    ...
    switch (s->blockState) {
        ...
        case TBLOCK_INPROGRESS:
            ereport(
                ERROR,
                (errcode(ERRCODE_S_E_INVALID_SPECIFICATION), errmsg(
                    "no such savepoint")));
            break;
...
void RollbackAndReleaseCurrentSubTransaction(void) {
    ...
    s = CurrentTransactionState; /* changed by pop */
    AssertState(
        s->blockState == TBLOCK_SUBINPROGRESS || s->blockState ==
        TBLOCK_INPROGRESS ||
        s->blockState == TBLOCK_STARTED);
}
...
char* GetSavepointName(List* options) {
    ...
    AssertEreport(PointerIsValid(name), MOD_TRANS_XACT,
 ...
 */
void RecordSavepoint(const char* cmd, const char* name, bool hasSent,
    ...
    savepointInfo = (SavepointData*)MemoryContextAlloc(
        u_sess->top_transaction_mem_cxt, sizeof(SavepointData));
    savepointInfo->cmd = MemoryContextStrdup(u_sess->top_transaction_mem_cxt,
                                             cmd);
    savepointInfo->name = MemoryContextStrdup(u_sess->top_transaction_mem_cxt,
                                              name);
...
void SendOneSavepointToRemoteCoordinators(const char* cmd, const char* name,
                                          SavepointStmtType stmtType,
                                          GlobalTransactionId transactionId) {
    switch (stmtType) {
        case SUB_STMT_SAVEPOINT:
            pgxc_node_remote_savepoint(cmd, EXEC_ON_COORDS, true, true,
                                       transactionId);
            break;
        case SUB_STMT_RELEASE:
            pgxc_node_remote_savepoint(cmd, EXEC_ON_COORDS, false, false,
        ...
        default:
            ereport(
                ERROR,
                (errmsg("Wrong type: %d in execSendSavepoint.", stmtType)));
            break;
    }
}
...
void SendSavepointToRemoteCoordinator() {
    ...
    dlist_foreach_cell(cell, dlist) {
        ...
        if (elem->hasSent == false) {
            elem->hasSent = true;
            SendOneSavepointToRemoteCoordinators(
                elem->cmd, elem->name, elem->stmtType, elem->transactionId);
        }
    }
}
 ...
 */
void HandleReleaseOrRollbackSavepoint(const char* cmd, const char* name,
                                      SavepointStmtType stmtType) {
    DList** dlist = &TopTransactionStateData.savepointList;
    ...
    while (cell != NULL) {
        SavepointData* elem = (SavepointData*)lfirst(cell);
        AssertEreport(PointerIsValid(elem), MOD_TRANS_XACT,
                      "Savepoint pointer is invalid");
        ...
        if (strcmp(elem->name, name) != 0) {
            ...
            if (stmtType == SUB_STMT_ROLLBACK_TO) {
                ...
                ereport(
                    ERROR,
                    (errmsg("Wrong type: %d when handling savepoints.", stmtType
                    )));
            }
        }
...
void AbortOutOfAnyTransaction(bool reserve_topxact_abort) {
    ...
    do {
        ...
        if (reserve_topxact_abort && (s->parent == NULL) && (
                s->blockState == TBLOCK_ABORT)) {
            break;
        }
...
bool IsTransactionBlock(void) {
    ...
    if (u_sess->SPI_cxt.portal_stp_exception_counter > 0 && s->blockState ==
        TBLOCK_SUBINPROGRESS) {
        return false;
    }
...
char TransactionBlockStatusCode(void) {
    ...
    ereport(FATAL, (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                errmsg("invalid transaction block state: %s", BlockStateAsString
                    (s->blockState))));
...
static void StartSubTransaction(void) {
    ...
    if (s->state != TRANS_DEFAULT) {
        ereport(WARNING,
                (errmsg("StartSubTransaction while in %s state",
                    TransStateAsString(s->state))));
    }
    ...
    Assert(
        !(IsAutoVacuumWorkerProcess() && (t_thrd.pgxact->vacuumFlags &
            PROC_IN_VACUUM)));

    /* Begin transaction and get timestamp but not gxid from GTM */
    if (!u_sess->attr.attr_common.xc_maintenance_mode && IS_PGXC_COORDINATOR &&
        !IsConnFromCoord()) {
...
static void CommitSubTransaction(bool STP_commit) {
    ...
    if (s->state != TRANS_INPROGRESS) {
        ereport(WARNING,
                (errmsg("CommitSubTransaction while in %s state",
                    TransStateAsString(s->state))));
    }
    ...
    AtEOSubXact_RelationCache(true, s->subTransactionId,
                              s->parent->subTransactionId);
    AtEOSubXact_PartitionCache(true, s->subTransactionId,
                               s->parent->subTransactionId);
    AtEOSubXact_BucketCache(true, s->subTransactionId,
                            s->parent->subTransactionId);
    ...
    if (STP_commit) {
        ResourceOwnerDecrementNPlanRefs(t_thrd.utils_cxt.CurrentResourceOwner,
                                        true);
        ResourceOwnerDecrementNsnapshots(t_thrd.utils_cxt.CurrentResourceOwner,
                                         NULL);
    }
    ...
    ResourceOwnerRelease(s->curTransactionOwner, RESOURCE_RELEASE_LOCKS, true,
                         false);
    ResourceOwnerRelease(s->curTransactionOwner, RESOURCE_RELEASE_AFTER_LOCKS,
                         true, false);
    ...
    t_thrd.utils_cxt.CurTransactionResourceOwner = s->parent->
        curTransactionOwner;
...
void AbortSubTransaction(bool STP_rollback) {
    ...
    if (s->state != TRANS_INPROGRESS) {
        ereport(WARNING,
                (errmsg("AbortSubTransaction while in %s state",
                    TransStateAsString(s->state))));
    }
    ...
    if (s->curTransactionOwner) {
        if (GTM_MODE && IS_PGXC_COORDINATOR && !IsConnFromCoord()) {
            if (!AtEOXact_GlobalTxn(false, false)) {
                ereport(WARNING, (errcode(ERRCODE_IN_FAILED_SQL_TRANSACTION),
                            errmsg(
                                "Failed to receive GTM abort subtransaction response."
                            )));
            }
        }
        ...
        AtEOSubXact_RelationCache(false, s->subTransactionId,
                                  s->parent->subTransactionId);
        AtEOSubXact_PartitionCache(false, s->subTransactionId,
                                   s->parent->subTransactionId);
        AtEOSubXact_BucketCache(false, s->subTransactionId,
                                s->parent->subTransactionId);
...
void CleanupSubTransaction(void) {
    ...
    if (s->state != TRANS_ABORT) {
        ereport(WARNING,
                (errmsg("CleanupSubTransaction while in %s state",
                    TransStateAsString(s->state))));
    }
...
static void PushTransaction(void) {
    ...
    s = (TransactionState)MemoryContextAllocZero(
        u_sess->top_transaction_mem_cxt, sizeof(TransactionStateData));
...
static void PopTransaction(void) {
    ...
    if (s->state != TRANS_DEFAULT) {
        ereport(WARNING,
                (errmsg("PopTransaction while in %s state", TransStateAsString(s
                    ->state))));
    }
    if (s->parent == NULL) {
...
static void ShowTransactionStateRec(TransactionState s) {
    ...
    ereport(DEBUG3,
            (errmsg_internal(
                "name: %s; blockState: %13s; state: %7s, xid/subid/cid: "
                XID_FMT "/" XID_FMT
                "/%u%s, nestlvl: %d, children: %s",
                PointerIsValid(s->name) ? s->name : "unnamed",
                BlockStateAsString(s->blockState),
                TransStateAsString(s->state), s->transactionId, s->
                subTransactionId,
                (unsigned int)t_thrd.xact_cxt.currentCommandId,
                t_thrd.xact_cxt.currentCommandIdUsed ? " (used)" : "", s->
                nestingLevel, buf.data)));
...
HTAB* relfilenode_hashtbl_create() {
    ...
    hashtbl = hash_create("relfilenode_hashtbl_create", 64, &hashCtrl,
...
static void unlink_relfiles(_in_ ColFileNodeRel* xnodes, _in_ int nrels,
    ...
    for (int i = 0; i < nrels; ++i) {
        ...
        if (!IsValidColForkNum(colFileNode.forknum)) {
            ...
            if (IS_PGXC_COORDINATOR && IsValidPaxDfsForkNum(
                    colFileNode.forknum)) {
             ...
             */
            if (!IS_PGXC_COORDINATOR && IsTruncateDfsForkNum(
                    colFileNode.forknum)) {
            ...
            ColumnRelationDoDeleteFiles(&colFileNode.filenode,
                                        colFileNode.forknum, InvalidBackendId,
                                        colFileNode.ownerid);
            UnlockRelFileNode(relFileNode, AccessExclusiveLock);
        }
    }
 ...
 */
static void xact_redo_commit_internal(TransactionId xid, XLogRecPtr lsn,
                                      TransactionId* sub_xids, int nsubxacts,
                                      SharedInvalidationMessage* inval_msgs,
                                      int nmsgs, ColFileNodeRel* xnodes,
                                      bool hasbucket, int nrels, int nlibrary,
                                      Oid dbId, Oid tsId, uint32 xinfo,
                                      uint64 csn,
                                      TransactionId newStandbyXmin) {
    ...
    if (TransactionIdFollowsOrEquals(max_xid,
                                     t_thrd.xact_cxt.ShmemVariableCache->
                                     nextXid)) {
    ...
    if (t_thrd.xlog_cxt.standbyState == STANDBY_DISABLED) {
         ...
         */
        CLogSetTreeStatus(xid, nsubxacts, sub_xids, CLOG_XID_STATUS_COMMITTED,
                          InvalidXLogRecPtr);
        ...
        if (csn >= COMMITSEQNO_FROZEN) {
            ...
            ereport(
                PANIC, (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                    errmsg("xact_redo_commit_internal: unknown csn state %lu", (
                        uint64)csn)));
        }
#endif
    } else {
    ...
    if (nrels > 0) {
        ...
        ((!XLogRecPtrIsInvalid(globalDelayDDLLSN) && XLByteLT(
              globalDelayDDLLSN, lsn))
             ? true
             : false);
    ...
    if (nlibrary > 0) {
        ...
        char* filename = (char*)xnodes + (nrels * sizeof(ColFileNodeRel)) + (nsubxacts * sizeof(TransactionId)) +
                         (nmsgs * sizeof(SharedInvalidationMessage));
#else
        char* filename = (char*)xnodes + (nrels * sizeof(ColFileNodeRel)) + (
                             (nsubxacts + 1) * sizeof(TransactionId)) +
                         (nmsgs * sizeof(SharedInvalidationMessage));
#endif
...
static void xact_redo_abort(xl_xact_abort* xlrec, TransactionId xid,
    ...
    if (xlrec->nlibrary) {
        ...
        char* filename = NULL;
        filename = (char*)xlrec->xnodes + (
                       (unsigned)xlrec->nrels * sizeof(ColFileNodeRel)) +
...
void xact_redo(XLogReaderState* record) {
    ...
    if (info == XLOG_XACT_COMMIT_COMPACT) {
        xl_xact_commit_compact* xlrec = (xl_xact_commit_compact*)
        ...
        xl_xact_commit_prepared* xlrec = (xl_xact_commit_prepared*)
            XLogRecGetData(record);
        ...
        xl_xact_abort_prepared* xlrec = (xl_xact_abort_prepared*)
            XLogRecGetData(record);
        ...
        PrepareRedoRemove(xlrec->xid, false);
        LWLockRelease(TwoPhaseStateLock);
    } else if (info == XLOG_XACT_ASSIGNMENT) {} else {
        ereport(PANIC,
                (errcode(ERRCODE_INVALID_TRANSACTION_STATE), errmsg(
                    "xact_redo: unknown op code %u", (uint32)info)));
    }
}

void XactGetRelFiles(XLogReaderState* record, ColFileNodeRel** xnodesPtr,
                     int* nrelsPtr) {
    ...
    switch (info) {
        ...
        case XLOG_XACT_COMMIT_PREPARED:
            commit = &(((xl_xact_commit_prepared*)XLogRecGetData(record))->
                crec);
            break;
        ...
        default:
            ereport(PANIC, (errmodule(MOD_REDO), errcode(ERRCODE_LOG),
                        errmsg("xactWillRemoveRelFiles: unknown op code %u", (
                            uint32)info)));
    }
...
void ForgetTransactionLocalNode(void) {
    t_thrd.xact_cxt.XactReadLocalNode =
        t_thrd.xact_cxt.XactWriteLocalNode = false;
}
...
bool IsPGXCNodeXactDatanodeDirect(void) {
     ...
     */
    return IS_PGXC_DATANODE && (IsPostmasterEnvironment || !useLocalXid) &&
           IsNormalProcessingMode() &&
           !IsAutoVacuumLauncherProcess() && !IsConnFromCoord() && !
           IsConnFromDatanode() && !IS_SINGLE_NODE;
}
#endif
...
void StreamTxnContextSaveXact(StreamTxnContext* stc) {
    ...
    STCSaveElem(stc->subTransactionId,
                CurrentTransactionState->subTransactionId);
    STCSaveElem(stc->currentSubTransactionId,
                t_thrd.xact_cxt.currentSubTransactionId);
    ...
    STCSaveElem(stc->GTMxactStartTimestamp,
                t_thrd.xact_cxt.GTMxactStartTimestamp);
    STCSaveElem(stc->stmtSystemTimestamp,
                t_thrd.time_cxt.stmt_system_timestamp);
}

void StreamTxnContextRestoreXact(StreamTxnContext* stc) {
    STCRestoreElem(stc->subTransactionId,
                   CurrentTransactionState->subTransactionId);
    STCRestoreElem(stc->currentSubTransactionId,
                   t_thrd.xact_cxt.currentSubTransactionId);
    ...
    STCRestoreElem(stc->GTMxactStartTimestamp,
                   t_thrd.xact_cxt.GTMxactStartTimestamp);
    STCRestoreElem(stc->stmtSystemTimestamp,
                   t_thrd.time_cxt.stmt_system_timestamp);
}

void StreamTxnContextSetTransactionState(StreamTxnContext* stc) {
    TransactionState srcTranState = (TransactionState)stc->
        CurrentTransactionState;
    TransactionState s = CurrentTransactionState;
    /*
 ...
 * @return - no return
 */
void ExtendCsnlogForSubtrans(TransactionId parent_xid, int nsub_xid,
...
CommitSeqNo SetXact2CommitInProgress(TransactionId xid, CommitSeqNo csn) {
    ...
    nchildren = xactGetCommittedChildren(&children);
    CSNLogSetCommitSeqNo(xid, nchildren, children,
                         COMMITSEQNO_COMMIT_INPROGRESS | latestCSN);

    ereport(
        DEBUG1,
        (errmsg("Set %lu to commit in progress, latest csn is %lu", xid,
            latestCSN)));
    return latestCSN;
}
...
TransactionState CopyTxnStateByCurrentMcxt(TransactionState state) {
    ...
    TransactionState ret = (TransactionState)palloc(
        sizeof(TransactionStateData));
    errno_t code = memcpy_s(ret, sizeof(TransactionStateData), state,
                            sizeof(TransactionStateData));
    securec_check(code, "", "");
    ...
    if (state->childXids) {
        ret->childXids = (TransactionId*)palloc(
